...
class Reconnect_node():
	def __init__(self):
		...
		# -- PUB
		self.pub_statusReconnect = rospy.Publisher('/status_reconnectDriver', Status_reconnect, queue_size= 10)
		...
		self.topicSub_loadcell = "/loadcell_respond"
		self.isRuned_loadcell = 0
		self.timeReaded_loadcell = time.time()
		rospy.Subscriber(self.topicSub_loadcell, Loadcell_respond, self.callback_loadcell)
		# --
		self.reconnect_loadcell = Reconnect(self.topicSub_loadcell, self.timeLost_loadcell, self.timeWait_loadcell, self.path_loadcell)
		# ---------------------------------------------- Close driver ALL ----------------------------
		# ---------------------
		# -- -- Parking
		...
		self.timeLost_goalControl = rospy.get_param("timeLost_goalControl", 3)
		self.timeWait_goalControl = rospy.get_param("timeWait_goalControl", 5)
		self.topicSub_goalControl = "/status_goal_control"		
		self.isRuned_goalControl = 0
		self.timeReaded_goalControl = time.time()
		rospy.Subscriber(self.topicSub_goalControl, Status_goal_control, self.callback_goalControl)
		# -- 
		self.reconnect_goalControl = Reconnect(self.topicSub_goalControl, self.timeLost_goalControl, self.timeWait_goalControl, self.path_goalControl)
		# ---------------------

	def callback_goalControl(self, data):
		self.isRuned_goalControl = 1
		self.timeReaded_goalControl = time.time()
	...
	def reconnect_node(self):
		...
		# -- loadcell
		# process, num = self.reconnect_loadcell.run_reconnect_vs2(self.isRuned_loadcell, self.timeReaded_loadcell)
		# self.statusReconnect.loadcell.sts = process
