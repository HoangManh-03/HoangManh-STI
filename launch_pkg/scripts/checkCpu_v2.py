...
class NUC_Info:
    ...
    def check_status_linuxwifi(self):
        try:
            ...
            result = subprocess.run(['nmcli', 'radio', 'wifi'], stdout=subprocess.PIPE, text=True, timeout = 10)
            
            wifi_status = result.stdout.strip()
            
            # Nếu Wi-Fi đang tắt, bật lại
            if wifi_status == 'disabled':
                print("Wi-Fi đang tắt")
                return 0
    ...
    def run(self):
        while not rospy.is_shutdown():
            ...
            # List wifi NUC can connect currently
            try:
                all_network = self.scan_networks()
                
                if all_network != "":
                    self.networks = self.parse_networks(all_network)
                    self.filtered_networks = self.filter_networks(self.networks, self.SSID)
                    
                    for i in self.filtered_networks:
                        ...
                        self.list_data_wifi.append(new_data_w)
                else:
                    self.wifi_info.err = 1    # Danh sachs wifi trong
                    self.list_data_wifi = []
                    self.allow_auto_connectWifi = 0
                    self.process = 1
                    print("List Wifi empty")
                    
            except Exception as e:
                self.wifi_info.err = 333        # Loi khi quet wifi
            ...
            # Function: Auto connect Wifi when wifi signal is low
            if self.allow_auto_connectWifi: 
                self.wifi_info.status_autoconnect = 1

                # Process 0: Toggle wifi if condition fulfill
                if self.process == 0:
                    if self.step_sys == 0:
                        self.ct_wait_wifion = rospy.get_time()
                        self.Control_LinuxWifi(False)
                        self.step_sys = 1

                    elif self.step_sys == 1:
                        if rospy.get_time() - self.ct_wait_wifion > self.TIME_WAIT_WIFION:
                            self.Control_LinuxWifi(True)
                            self.ct_wait_wifioff = rospy.get_time()
                            self.step_sys = 2

                    elif self.step_sys == 2:
                        if rospy.get_time() - self.ct_wait_wifioff > self.TIME_WAIT_WIFIOFF:
                            self.step_sys = 0
                            self.process = 1

                    self.wifi_info.nb_toggle_wifi += 1

                # Process 1: Check status of Linux control Wifi
                elif self.process == 1:
                    status_cardwifi = self.check_status_linuxwifi()

                    # -- Case 1: Wifi is off >> switch on wifi
                    ...
                    # -- Case 2: Wifi is on
                        ...
                        if self.is_connectFirstTime == True:
                            ssid, bssid = self.get_wifi_info(self.name_card)      
                            print("MAC:", bssid)
                            if ssid != None:
                                self.process = 2
                                self.is_connectFirstTime = False
                        else:
                            self.process = 2


                # Process 2: Get info Wifi which Linux is connected
                elif self.process == 2:          
                    ssid, bssid = self.get_wifi_info(self.name_card)      
                    print("MAC:", bssid)
                    
                    # Case 1: Find a valid wifi
                    if ssid != None:                     
                        ...
                        if ssid != self.SSID:
                            ...
                            if not connect_status:
                                self.process = 0
                                self.step_sys = 0
                                print("Kết nối ko thành công với Wifi >> tăt wifi đi bật lại")
                    ...
                    # Case 2: Cannot find valid wifi >> lost wifi >> toglle wifi
                    else:
                        self.wifi_info.status_wifi = False
                        self.process = 0
                        self.step_sys = 0
                        print("Ko thể lấy or ko tìm thấy wifi nào >> tăt wifi đi bật lại")

                # Process 3: Check signal of current Wifi >> Try other wifi if low signal
                elif self.process == 3:
                    self.process = 2
                    
                    # -- Get signal of current wifi
                    self.singal_now = -1
                    for info_ap in self.filtered_networks:
                    ...
                    # - Compare current signal wifi with threshold signal
                    if self.singal_now != -1 and self.singal_now <= self.roaming_intensity and self.singal_now < int(self.filtered_networks[0][2]):
                        self.MAC_APSELECT = self.filtered_networks[0][1]
                        print(f"Thay doi AP co MAC: {self.MAC_APSELECT}")
                        self.process = 4

                # Process 4: Connect to Wifi with mac exactly
                elif self.process == 4:
                    ...
                    if not status_connect:
                        ...
                        self.wifi_info.nb_change_macwifi += 1
                        self.process = 1
                        print("Da thay doi wifi theo mac khác ^^^^^")
            else:
