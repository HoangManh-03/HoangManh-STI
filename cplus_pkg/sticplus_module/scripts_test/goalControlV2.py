...
import numpy as np
from math import sqrt, pow, atan, fabs, cos, sin, acos
...
class GoalControl(threading.Thread):
    ...
    def checkPathOutOfRange(self):
        if self.isGoalNearest :
            ...
            if self.stepCheckStart == 0:
                if fabs(angleBW) <= 30.*PI/180.:
                    self.pub_Stop()
                    self.stepCheckStart = 1

                else:
                    twist = Twist()
                    if angleBW > 0.: # quay trai
            ...
            if self.stepCheckStart == 1:
                if self.disRbToGoalNearest <= 0.02  and fabs(angleBW) <= 10.*PI/180.:
    ...
    def getWaitPoint(self, xRb, yRb, curr_velocity):
        ...
        if indexSelect == -1:
            ...
            if self.flagCheckAngle:
                ...
                if fabs(angle) <= 0.02 and self.disRbToGoalNearest <= 0.017:
                    self.flagCheckAngle  = False
    ...
    def followPath(self):
        ...
        print(ss_x, ss_y, dis)
        if ((fabs(ss_x) <= 0.01 and fabs(ss_y) <= 0.01) or dis <= 0.02) and self.flagFollowPointFinish:
            self.pub_Stop()
            return 2
