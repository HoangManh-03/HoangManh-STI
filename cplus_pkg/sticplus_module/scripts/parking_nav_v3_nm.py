...
class ParkingNAV(threading.Thread):
    ...
    # Quy đổi frame nav về frame điểm target, điểm target sẽ có vị trí là 0,0 
    # trả về vị trí của frame nav theo frame id 
    # mục tiêu làm sao cho 2 frame này trùng nhau, tức là tọa độ của frame_nav = 0,0
    ...
    def transformPoseNAV(self, frame_id_pointTarget, frame_agvNAV):   
        ...
        try:
            ...
            self.ss_x = pos[0]                        # khoảng sai số giữa frame agv và frame target 
            self.ss_y = pos[1]
    ...
    def move_forward(self, S, direct, velocity_min, permission_tolerance):
        ...
        if self.step_moveForward == 0:
            ...
            if self.req_parking.modeRun == 1 or self.req_parking.modeRun == 2 or self.req_parking.modeRun == 3:
        ...
        if poseThenTransform.position.x == 0.0 and poseThenTransform.position.y == 0.0 and poseThenTransform.orientation.z == 0.0 and poseThenTransform.orientation.w == 0.0 :
            ...
            if self.req_parking.modeRun == 1 or self.req_parking.modeRun == 2 or self.req_parking.modeRun == 3:
                if distance >= permission_tolerance and poseThenTransform.position.x < permission_tolerance: # dieu kien den target
                    if (self.zone_lidar.zone_sick_behind == 1 and self.req_parking.modeRun == 3) or (self.zone_lidar.zone_sick_behind != 0 and self.req_parking.modeRun == 1):
                        self.warn_agv = 1
                        # print("co vat can")
                        self.pub_Stop()
        ...
        while not self.shutdown_flag.is_set():
            ...
            if self.process == 0:
                ...
                if self.is_request_parking == True:
                    if self.req_parking.modeRun == 1 or self.req_parking.modeRun == 2 or self.req_parking.modeRun == 3: #Run
                ...
                stt = self.follow_target(self.min_velFinish, self.max_vel, self.min_rolFinish, 0.6, 0.4, 0)
                if stt == 1:  # 0.5 | 0.35
                    # print("Done Parking!")
                    time.sleep(0.5)
                    self.process = 51
...
def main():
    ...
    try:
        ...
        while not rospy.is_shutdown():
            if j1.pubTransform == True and (j1.req_parking.modeRun == 1 or j1.req_parking.modeRun == 2 or j1.req_parking.modeRun == 3):
                j1.sendTransform('frame_map_nav350', 'frame_target', j1.req_parking.poseTarget)
