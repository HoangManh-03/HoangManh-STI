...
class goalControl():
    def __init__(self):
        ...
        self.vel_x_max = rospy.get_param('~vel_x_max',0.65)
        self.vel_x_min = rospy.get_param('~vel_x_min',0.04)
        ...
        self.dist_ahead_max = rospy.get_param('~khoang_nhin_truoc_max',1.5) # 1.4
        self.dist_ahead_min = rospy.get_param('~khoang_nhin_truoc_min',0.9)
        self.khoang_offset = rospy.get_param('~khoang_offset', 1.)
        self.distance_goArc = rospy.get_param('~distance_goArc', 2.0)
        self.needRotatyFinish = rospy.get_param('~need_rotaty_finish', 1)

        self.MaxRadiusCircle = rospy.get_param('~MaxRadiusCircle', 1.5) # 2.
        ...
        self.flagAccelerationModeNormal = False
        self.is_pre_pttt = 0
        self.x_td_goal = 0.0 
        self.y_td_goal = 0.0
        self.dis_hc = 0.0
        ...
        # variable tinh van toc khi lech goc voi dt follow
        self.velOutOfAngle = 0.2
        self.theta = 0.0

        self.angle_find_vel = 25.0*PI/180.0
        self.saveTime = rospy.Time.now().to_sec()

        self.angle_giam_toc = 45.0*PI/180.0

        self.distanceGiamToc = 1.1
        self.distanceGiamTocKhiLui = 0.3
        self.kc_con_lai = 0.0
        self.kc_qd = 0.0
        ...
        # bien update follow theo hinh tron
        self.X_Circle = 0.
        self.Y_Circle = 0.
        self.RadiusFollow = 0.
        ...
        self.offsetFollowCircle = 0.02

        self.velFollowCircle = 0.25
        self.distanceAcceleration = 0.75
        self.flagAccelerationModeCircle = False
        self.valueAcceleration = 0.15
    ...
    def find_point_special(self, x_c_goal, y_c_goal, x_n_goal, y_n_goal, dk_check):
        ...
        if dk_check == 1:
            if goc >= 75.0*PI/180.0 and goc < 160.0*PI/180.0:
    ...
    def update_all(self):
        ...
        if len(self.req_move.list_x) == 5 and len(self.req_move.list_y) == 5 and len(self.req_move.list_id) == 5 and len(self.req_move.list_speed) == 5 and self.req_move.list_id[0] != 0.0:
            ...
            self.velFromTraffic = self.req_move.list_speed[0]
    ...
    def phuongTrinhGiaTocWithAcc(self, time_s, v_s, v_f, a):
        v_re = 0.0
        flagDone = False
        if v_s > v_f:
            a = -a
        ...
        return v_re, flagDone
    
    def vantocModeCircle(self, saveVelCmdSend, velFollow):
        vel = velFollow
        if self.flagAccelerationModeCircle == True:
            vel, isDone = self.phuongTrinhGiaTocWithAcc(self.saveTime, saveVelCmdSend, velFollow, self.valueAcceleration)
            if isDone == True:
                vel = self.velFollowCircle
                self.flagAccelerationModeCircle = False

        return vel

    def vantocModeNormal(self, deviationAngle, disRobotToGoal):
        velConvertTraffic = 0.0
        velCalAngle = 0.0
        velCalPTGiaToc = 0

        # new traffic
        if self.velFromTraffic == 0: # roi vao th khong xac dinh
            velConvertTraffic = 0.3
            
        else:
            velConvertTraffic = round((self.velFromTraffic/120.0)*self.vel_x_max,3)
            
        if velConvertTraffic > self.vel_x_max:
            velConvertTraffic = self.vel_x_max

        # tinh van toc theo goc lech so voi duong thang follow
        if fabs(deviationAngle) > self.angle_find_vel:
            velCalAngle = self.velOutOfAngle

        elif round(fabs(deviationAngle),3) == 0.0:
            velCalAngle = velConvertTraffic

        else:
            velCalAngle = self.velOutOfAngle + ((self.angle_find_vel - fabs(deviationAngle))/self.angle_find_vel)*(velConvertTraffic - self.velOutOfAngle)


        if self.flagAccelerationModeNormal == True and disRobotToGoal > self.distanceGiamToc:
            self.is_pre_pttt = 1
            self.vel_x_now = self.phuongTrinhGiaToc(3.0,self.saveTime,0.0, velCalAngle)
            velCalPTGiaToc = self.vel_x_now
            if velCalPTGiaToc >= velCalAngle:
                self.flagAccelerationModeNormal = False
                self.is_pre_pttt = 0
                velCalPTGiaToc = velCalAngle
                # print("1")

        else:
            self.flagAccelerationModeNormal = False
            if self.is_pre_pttt == 1 and self.vel_x_now >= 0.3:
                velCalPTGiaToc = self.vel_x_now*(disRobotToGoal/self.distanceGiamToc)
                # print("2")

            else:
                self.is_pre_pttt = 0
                velCalPTGiaToc = velCalAngle*(disRobotToGoal/self.distanceGiamToc)    
                # print("3")
                
            if velCalPTGiaToc > velCalAngle:
                velCalPTGiaToc = velCalAngle

        return velCalPTGiaToc
    ...
    def findGoalValid(self, list_x, list_y, list_id):
        id = 0
        listDistanPointGoalToAGV = []
        for i in range(len(list_id)):
            if list_id[i] != 0:
                dis = self.fnCalcDistPoints(list_x[i], self.poseRbMa.position.x, list_y[i], self.poseRbMa.position.y)
                listDistanPointGoalToAGV.append(dis)
            else:
                break
        numId = len(listDistanPointGoalToAGV)
        if numId > 1:
            tg_dis = 0
            tg_id = 0
            for i in range(0, numId-1, 1):
                for j in range(i+1, numId, 1):
                    if listDistanPointGoalToAGV[i] > listDistanPointGoalToAGV[j]:
                        tg_dis = listDistanPointGoalToAGV[i]
                        listDistanPointGoalToAGV[i] = listDistanPointGoalToAGV[j]
                        listDistanPointGoalToAGV[j] = tg_dis
        ...
        while not rospy.is_shutdown():
            ...
            # tim ra current goal, next goal
                ...
                if self.stt_agv == 0 : # AGV tinh goal tiep theo sau khi dung
                        ...
                        if self.is_target_change == True:
                            ...
                            if self.id_fl != 0.0:   
                                ...
                                if _kcRobottoGoal > self.ss_luivsnextGoal:
                                    ...
                                    self.flagAccelerationModeNormal = True
                                    ...
                                    if round(self.cur_goal_x, 3) == round(self.target_x, 3) and round(self.cur_goal_y, 3) == round(self.target_y, 3):
                                        ...
                                        if self.list_id[1] != 0.0:
                                            ...
                                            self.velFromTraffic = self.list_vel[1]
                                            self.process = 6
                            ...
                            if numberPoint == 0 or numberIsValid == 0:
                                self.end_of_list = True
                                self.process = 2
                                ...
                                self.velFromTraffic = self.list_vel[_stt+1]
                                self.process = 6
                            ...
                            #         self.velFromTraffic = self.list_vel[0]
                            #         self.process = 6
                            ...
                            #         self.velFromTraffic = self.list_vel[f_next]
                            #         self.process = 6
                    ...
                    if self.cur_goal_is < 2:
                        if (round(self.cur_goal_x, 3) == round(self.target_x, 3)) and (round(self.cur_goal_y, 3) == round(self.target_y, 3)):
                            ...
                            if numberIsValid == 0 or numberPoint == 0:
                                if kc < self.khoang_offset: # dung xet dieu kien khi AGV gan goal
                                ...
                                if self.checkedCircle == False:
                                    if _d1 > self.distance_goArc:
                                        rospy.logwarn("Dieu kien khoang cach 1 OKE!")
                                        if _d2 > self.distance_goArc:
                                            rospy.logwarn("Dieu kien khoang cach 2 OKE!")
                                            if _theta3Point >= 88.0*PI/180.0 and _theta3Point < 160.0*PI/180.0:
                                                rospy.logwarn("DK goc giua 2 duong thang OKE!")
                                ...
                                if self.isTurnCircle == False:
                                    ...
                                    if self.startTurnCircle == False:
                                        ...
                                        if self.disRadiusFollowCircle > _kc7 + self.offsetFollowCircle :
                                            rospy.logwarn("START FOLLOW CIRCLE")
                                            self.startTurnCircle = True
                                            self.flagAccelerationModeCircle = True
                                            self.saveTime = rospy.Time.now().to_sec()
                                            self.saveVelCmdSend = self.velCmdSend

                                    else:
                                        if self.doneTurnCircle == False:
                    ...
                    #                         if (dist > 3.0 and goc > 165.0*PI/180.0):
                    #                             self.cur_goal_is = 1
                    #                             self.point_goal_start_x = self.cur_goal_x
                    #                             self.point_goal_start_y = self.cur_goal_y
                    #                             self.cur_goal_x = self.list_x[1]
                    ...
                    #                                 self.velFromTraffic = self.list_vel[f_next + 1]
                    #                         else:
                    #                             rospy.logwarn("OUT CHECK GO ARC!!!, MODE 3")
                    #                     else:
                    #                         rospy.logwarn("OUT CHECK GO ARC!!!")
            ...
            #----- dieu huong AGV
                ...
                if self.startTurnCircle == True and self.doneTurnCircle == False:
                    ...
                    v_x_send = self.vantocModeCircle(self.saveVelCmdSend, self.velFollowCircle)
                    ...
                    self.distance_goal = self.fnCalcDistPoints(self.poseRbMa.position.x,\
                                                                        ...
                                                                        self.cur_goal_y)
                    v_x_send = self.vantocModeNormal(self.theta, self.distance_goal)
                ...
                if s_nav < fabs(self.kc_backward):
                    ...
                    # else:
                    # if self.zone_lidar.zone_sick_ahead == 1 or self.zone_lidar.zone_sick_ahead == 2 or zonenav == 1:
                    if self.zone_lidar.zone_sick_ahead == 1 or zonenav == 1:
                        print("co vat can")
                        self.war_agv = 1
                        self.stop()
                        ...
                        self.war_agv = 0
                        vel_lui = (fabs((fabs(self.kc_backward)- s_nav))/self.distanceGiamTocKhiLui)*0.16
                        if vel_lui > 0.16:
                            vel_lui = 0.16 
                        if vel_lui < 0.1:
                            vel_lui = 0.1
