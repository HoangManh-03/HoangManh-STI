    ...
    def run(self):
        ...
        while not self.shutdown_flag.is_set():
            ...
            if self.process == 0:
                ...
                kc = self.calculate_distance(self.poseRbMa.position.x, self.poseRbMa.position.y, self.req_parking.poseTarget.position.x, self.req_parking.poseTarget.position.y)
                # print(kc)
                if kc >= self.distance_accept:
                    self.warn_agv = 0
                    self.process = 41
                ...
                stt = self.follow_target(self.min_velFinish, self.max_vel, self.min_rolFinish, 0.7, 0.625, 0)
                if stt == 1:  # 0.5 | 0.35
                    # print("Done Parking!")
                    time.sleep(0.5)
                    self.process = 51
