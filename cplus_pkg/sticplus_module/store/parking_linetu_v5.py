...
class StiAutomaticParking(threading.Thread):
    ...
    def callSup1(self,data):
        self.Line_ahead = data.Line_ahead
        self.aBit = data.aBit
        ...
        while not self.shutdown_flag.is_set():
          ...
          #B31: check line tu 
            if buoc == 31 : 
                ...
                if self.line1 > 2. and self.line1 < 15. and math.fabs(x_target) < 0.5:
          ...
          #B9: tinh tien di vao = LINETU , NOTE : check yes/no Tag      
            if buoc == 9 :
                # self.park_info(buoc,find_id,0,0,0) 
                # rospy.loginfo("tt_x= %s , tt_z= %s, tt_g= %s",self.rb_x, self.rb_z, self.rb_g)
                # twist = Twist()
...
class Kalman_filter:
    ...
    def filter(self,input):
        ...
        self.x_ht = self.x_trc + self.k*(input-self.x_trc)
        self.p = (1-self.k)*self.p+ math.fabs(self.x_trc-self.x_ht)*self.q
        self.x_trc = self.x_ht    
        return self.x_ht
...
class ReadApriltag(threading.Thread):
    def __init__(self, threadID):
        ...
        # kalman
        self.tt_x = Kalman_filter(0.1, 1.0, 0.5)
        self.tt_x = Kalman_filter(0.1, 1.0, 0.5)
        self.tt_g = Kalman_filter(0.1, 1.0, 0.5)
    ...
    def fnFilterDataTag(self,id_tag): 
        ...
        if is_marker_pose_received == True :
            if find_id == id_tag :
                ...
                tt_x = self.tt_x.filter(position[0])
                tt_z = self.tt_z.filter(position[2])
                tt_g = self.tt_g.filter(theta) # radian
