...
int main(int argc, char** argv){
    ...
    while(n.ok()){
        ...
        tf2::Quaternion q2 ;  
        q2=q22*q21;               
        // ROS_INFO("q2: rx= %f , ry= %f , rz= %f , rw= %f",q2[0],q2[1],q2[2],q2[3]);        
        // double x_Rc = 0;
        // double y_Rc = 0.265;
        // double z_Rc = 0.27;
        double x_Rc = 0.32;
        ...
        double x_Ar1 = robot_map.position.x + ((y_Rc + aruco.position.x)*cos(yawArMc) - (x_Rc + aruco.position.z)*sin(yawArMc))  ;
        double y_Ar1 = robot_map.position.y + ((y_Rc + aruco.position.x)*sin(yawArMc) + (x_Rc + aruco.position.z)*cos(yawArMc))  ;
        double z_Ar1 = z_Rc + aruco.position.y*cos(rollAr1); 
