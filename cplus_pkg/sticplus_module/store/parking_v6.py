...
class StiAutomaticParking(threading.Thread):
    def __init__(self):
        ...
        # self.sub_odom_robot = rospy.Subscriber('/raw_odom', Odometry, self.cbGetRobotOdom, queue_size = 1) 
        ...
        self.dolech_y_CamVsRObot = 0.55 # m
    ...
    def my_kalman(self,r,p,q,input):

        self.k=p/(p+r)
        self.x_ht = self.x_truoc + self.k*(input-self.x_truoc)
        p=(1-self.k)*p+ math.fabs(self.x_truoc-self.x_ht)*q
        self.x_truoc = self.x_ht
        
        # print "r=",r
        # print "k = " , self.k
        # print "input =" ,input
        # print "x_ht=" ,self.x_ht
    ...
    def fnGetDataTag_old(self,id_tag,sl_loc): 
        tt_x = tt_z = tt_g = .0
        x_loc = .0
        dem = 0
        ...
        for dem in range(sl_loc):
            while self.is_marker_pose_received != True :
                ...
                # print "x :", position[0]
                # print "x_loc" ,self.my_kalman(10.0, 1.0, 0.01, position[0])
                # x_loc = x_loc + self.my_kalman(10.0, 1.0, 0.01, position[0])
                tt_x = tt_x + self.my_kalman(10.0, 1.0, 0.01, position[0])
    ...
    def run(self):
        ...
        while not self.shutdown_flag.is_set():
          ...
          #Test          
            if buoc == 0 :
                self.tt_x, self.tt_z, self.tt_g = self.fnGetDataTag(self.MARKER_ID_DETECTION,20)
                # print "odom_g = ", self.odom_g
                # self.tt_x, self.tt_z, self.tt_g = self.fnGetDataTag_notFilter(self.MARKER_ID_DETECTION) # rad
                # x_target_phu = self.tt_x - self.dolech_x_CamVsRObot
          ...
          #B2: Tinh goc can quay, tt_x , tt_z
            ...
            if buoc == 2 : 
                self.tt_x, self.tt_z, self.tt_g = self.fnGetDataTag(self.MARKER_ID_DETECTION,30) # rad
                print "tt_x= %f , tt_z= %f, tt_g= %f" %(self.tt_x, self.tt_z, self.tt_g)
                print "odom_g = ", self.odom_g
          ...
          #B7: tinh kc di vao       
            if buoc == 7 : 
                ...
                # if math.fabs(x_target) > 0.03: # dung sai +- 2cm
                #     buoc = 77
                # else : 
                buoc = 8
                # rospy.spin()
