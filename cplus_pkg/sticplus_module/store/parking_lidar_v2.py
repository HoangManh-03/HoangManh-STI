...
class Leg_detection(threading.Thread):
    def __init__(self, threadID):
        ...
        self.zone_pub = rospy.Publisher('zone_lidar',Zone_lidar, queue_size=1)
        ...
        while not self.shutdown_flag.is_set():
            if self.is_detec_leg == True :
                ...
                for i in range(len(self.legData.legs)):
                    ...
                    # print "i: ",i
                    #zon1 1
                    zone1 = self.check_zone(self.legData.legs[i].position, self.dx1, self.dy1)
                    if zone1 == True : dem_z1 = dem_z1 + 1
                    #zone 2
                    zone2 = self.check_zone(self.legData.legs[i].position, self.dx2, self.dy2)
                    if zone2 == True : dem_z2 = dem_z2 + 1
                    #zone 3
                    zone3 = self.check_zone(self.legData.legs[i].position, self.dx3, self.dy3)
                    if zone3 == True : dem_z3 = dem_z3 + 1

                if dem_z1 != 0 : 
                    dem_z1 = 0
                    self.zone.zone1 = True
                else : self.zone.zone1 = False

                if dem_z2 != 0 : 
                    dem_z2 = 0
                    self.zone.zone2 = True
                else : self.zone.zone2 = False

                if dem_z3 != 0 : 
                    dem_z3 = 0
                    self.zone.zone3 = True
                else : self.zone.zone3 = False
