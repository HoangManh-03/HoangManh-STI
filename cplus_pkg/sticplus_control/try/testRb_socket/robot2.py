...
class Communicate_socketIO():
	def __init__(self):
		rospy.init_node('sim_robot2', anonymous=False)
		...
		self.server_port = 4002

		self.AGV_IP = '192.168.1.100'
		self.AGV_port = 6000
		self.AGV_mac = "0c:9a:3c:07:bb:62"
		# -
		self.robot_x = 0.0
		self.robot_y = 1.5
		self.robot_r = radians(230)
		self.timeD = 1./8.
		# -
		self.vel_x = 0.3
		self.vel_r = 0.0
	...
	def run(self):
		...
		# if (self.robot_x > 3):
		# 	self.robot_r = radians(195)
		# 	self.vel_x = 0.2

		# if (self.robot_x < 0):
		# 	self.robot_r = radians(15)
		# 	self.vel_x = 0.15
		self.rate.sleep()
