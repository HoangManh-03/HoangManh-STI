...
// public:
    driver(ros::NodeHandle *nh, ros::NodeHandle *npr){
        ...
        ros::param::get("maxRPM", maxRPM);
        minRPM = maxRPM*(-1);
        
        ros::param::get("id_1", ID_driver1);
        ros::param::get("topicSub_1", topicSub_1);
        ros::param::get("topicPub_1", topicPub_1);
        ros::param::get("revert_1", revert_1);

        ros::param::get("id_2", ID_driver2);
        ros::param::get("topicSub_2", topicSub_2);
        ros::param::get("topicPub_2", topicPub_2);
        ros::param::get("revert_2", revert_2);

        // info pub
        pub_driverRespond_1 = nh->advertise<message_pkg::Driver_respond>(topicPub_1, 50);
        pub_driverRespond_2 = nh->advertise<message_pkg::Driver_respond>(topicPub_2, 50);

        // info sub
        sub_disable_brake = nh->subscribe("/disable_brake", 10, &driver::break_infoCallback, this);
        sub_driver1_query = nh->subscribe(topicSub_1, 50, &driver::driverQuery1_infoCallback, this);
        sub_driver2_query = nh->subscribe(topicSub_2, 50, &driver::driverQuery2_infoCallback, this);
    ...
    void run(Slave &drive, Master &mb, int rev, message_pkg::Driver_respond driverRespond, message_pkg::Driver_query driverQuery, statusDriver& sttDriver, ros::Publisher& rospb ){
        ...
        if (enable_run){
            spin_motor(drive, mb, driverQuery.rotationSpeed, rev);
        }
        else{
