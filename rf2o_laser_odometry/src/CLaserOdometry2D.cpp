  ...
  for (unsigned int i = 0; i<pyr_levels; i++)
  {
    ...
    if (i == 0)
    {
      for (unsigned int u = 0; u < cols_i; u++)
      {
        ...
        if ((u>1)&&(u<cols_i-2))
        {
          if (std::isfinite(dcenter) && dcenter > 0.f && dcenter <= 80.)
          {
            float sum = 0.f;
            float weight = 0.f;
...
void CLaserOdometry2D::PoseUpdate()
{
  ...
  // std::cout << "---" << std::endl;
  // std::cout << "v_x: " << kai_loc_old_(0) << " | v_y: " << kai_loc_old_(1) << " | end: " << kai_loc_old_(2) << std::endl;
  ...
  // last_m_lin_speeds_y.push_back(lin_speed_y);
  // if (last_m_lin_speeds_y.size()>4)
  //     last_m_lin_speeds_y.erase(last_m_lin_speeds_y.begin());
  // double sum1 = std::accumulate(last_m_lin_speeds_y.begin(), last_m_lin_speeds_y.end(), 0.0);
  // lin_speed_y = sum1 / last_m_lin_speeds_y.size();
