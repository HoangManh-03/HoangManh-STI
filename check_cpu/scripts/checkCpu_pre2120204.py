...
class NUC_Info:
    ...
    def connect_to_wifi_start(self):
        ...
        if "successfully activated" in result.stdout:
            ...
            with open('log.txt', 'a') as log_file:
                log_file.write(f'connect_to_wifi_start.---------- {current_time}\n')
            ...
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            with open('log.txt', 'a') as log_file:
                log_file.write(f'connect_fail_to_wifi_start.---------- {current_time}\n')
    ...
    def connect_to_wifi(self, mac_ap):
        ...
        if "successfully activated" in result.stdout:
            ...
            if str(self.get_current_wifi_mac()) == str(mac_ap):
                ...
                current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                with open('log.txt', 'a') as log_file:
                    log_file.write(f'connect_to_wifi.---------- {current_time}\n')
            ...
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            with open('log.txt', 'a') as log_file:
                log_file.write(f'connect_to_wifi_fail.---------- {current_time}\n')
            return 0

    def loop_connect_to_wifi(self):
        print("loop_connect_to_wifi")
        result = subprocess.run(['nmcli', 'dev', 'wifi', 'connect', self.SSID, 'password', str(self.PASS)], stdout=subprocess.PIPE, text=True, timeout=10)
        if "successfully activated" in result.stdout:
            # Kiểm tra nếu MAC của điểm truy cập
            subprocess.run(['nmcli', 'connection', 'modify', self.SSID, 'connection.autoconnect', 'no'], stdout=subprocess.PIPE, text=True)
            print(f"Đã kết nối thành công với MAC: {self.get_current_wifi_mac()}")

            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            with open('log.txt', 'a') as log_file:
                log_file.write(f'ket noi loop thanh cong.---------- {current_time}\n')
            return 1
        # else:
            # print("Kết nối không đúng với MAC yêu cầu.")
            # return 0
            ...
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            with open('log.txt', 'a') as log_file:
                log_file.write(f'ket noi loop that bai.---------- {current_time}\n')
            return 0
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # if self.process == 0:
            #     if self.button_status.wifi == 0:
            #         self.process = 0
            #         print("Khong lap lai tim kiem wifi")
            #     elif self.button_status.wifi == 1:
            #         print("Tim kiem lien tuc wifi")
            #         self.process = 1
            ...
            if self.process == 0:
                ...
                ssid, bssid = self.get_wifi_info(self.name_card)       # lấy thông tin wifi đang kết nối
                ...
                if ssid != None:
                    ...
                    if ssid != self.SSID:
                        ...
                        self.process = 2
                        if self.check_change_wifi == 1:                   # update lại cấu hình wifi lần đầu
                    ...
                    if all_network != "":                                                   #Nếu không có mạng nào sẽ kết nối với mạng có tín hiệu mạnh nhất hiện tại
                        ...
                        self.status_connect_wifi = self.loop_connect_to_wifi()     #Hàm kết nối mạng
                        self.configure_static_ip_wifi(self.MAC_APNOW, ssid)
                    ...
                    try:
                        ...
                        if all_network != "":
                         ...
                         # if self.signal_now != -1 and self.signal_now < self.roaming_intensity:
                            ...
                            self.wifi_info.err = 444         # ko tìm được wifi nào
                            
                    except Exception as e:                   # Lỗi tìm kiếm wifi
                        self.wifi_info.err = 333
                        print("Error executing find target wifi command: ", e)
