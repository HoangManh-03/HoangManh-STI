...
class MoveControl(LifecycleNode):
    def __init__(self):
        ...
        # -- follow Line 
        self.followLine3 = LineFollowerPID()
    ...
    def getVeloctity(self, velCmd, _statusVel):
        if _statusVel == 0:
            ...
            return self.funcDecelerationByAcc(self.saveTimeVel, self.velSt, velCmd, -0.3)
        else:
            return 0.
    ...
    def followLine(self, _dir, _safety_skip = False):
        ...
        if _dir == self.moveByHead:
            safety = self.zone_lidar.zone_sick_ahead

        elif _dir == self.moveByTail:
            safety = self.zone_lidar.zone_sick_behind
        ...
        if safety == 1:
            ...
            velCmd = 0.
            self.statusVel = 0
            self.infoAGV.war_agv = 1
            print('not safety')
        
        elif safety == 2:
            if self.isDecelerationObstacles == 1:
                self.saveVelWhenDecObs = velocity*(1./3.)
                self.statusVel = 0
            ...
            if velCmd <= self.vel_linear_min:
                velCmd = self.vel_linear_min
            ...
            if self.isDecelerationObstacles == 1 or self.isDecelerationObstacles == 2:
                self.saveVelWhenDecObs = velocity*(2./3.)
                self.statusVel = 0
        ...
        if self.curr_velocity != 0.:
            ...
            # status, vel_th = self.followMagline.calculate_value(self.curr_velocity, _dir, value_magline)
            # status, vel_th = self.followMagline2.calculate_value(self.curr_velocity, value_magline)
            status, vel_th = self.followLine3.calculate_value(self.curr_velocity, _dir, value_magline)

            if status == 0:
                self.stop()
                print("agv lech khoi line tu")
                self.infoAGV.err_agv = 1

            elif status == 1:
                twist = Twist()
                vel_linear = self.curr_velocity
                # vel_linear = vel_x
                if _dir == self.moveByTail:
    ...
    def doNextDirection(self, angle_turn):
        dir_turn = 1
        num_encounter1bit = 1
        time_offset = 0.
        time_turn = 0.
        ...
        if self.subProcessTurn == 0:
            ...
            if dir_turn == 0:
                ...
                time_move = time.time()- self.saveTimeWhenStop
                sensor_center = 1 if self.data_mangline_front.value >= 5 and self.data_mangline_front.value <= 10 else 0
    ...
    def run(self):
        ...
        self.pub_respondMove(self.mode_move, self.target_id, self.target_dir, 0, complete, self.infoAGV.err_agv, self.process, self.infoAGV.RFID_lastCode, self.infoAGV.RFID_code, self.infoAGV.dir_now)
