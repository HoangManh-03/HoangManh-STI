...
from math import atan2, sin, cos, sqrt , fabs, acos, radians, pow, degrees
...
class goalControl():
	def __init__(self):
		...
		self.khoang_offset = rospy.get_param('~khoang_offset', 0.8)
		self.check_circle = rospy.get_param('~vung_check_circle', 1) # 1: check trc sau | 2 check tat ca
		...
		self.vel_x1 = 0.3  		# level 1 slowless
		self.theta_max_1 = 0.4 

		self.vel_x2 = 0.4   		# level 2 
		self.theta_max_2 = 0.5

		self.vel_x3 = 0.5    		# level 3 fastless
	...
	def isNear_axis(self, point_robot, x, y, dis):
		d_x = abs(point_robot.x - x)
		d_y = abs(point_robot.y - y)
	...
	def calculateAngle_twoPoint(self, point_1, point_2x, point_2y): # point_1 | point_2 | geometry_msgs/Point | out | radian
		delta_x = point_2x - point_1.x
		delta_y = point_2y - point_1.y
		if (delta_x == 0):
			if (delta_y >= 0):
				return pi/2.
			else:
	...
	def findAngle_targetSecondary_vs1(self, pose_robot, pose_target_x, pose_target_y, angle_toGoal, delta_distance, distance_enb):
		angle_out = 0.0
		point_targetSecondary = Point()

		d_x = abs(pose_robot.position.x - pose_target_x)
		d_y = abs(pose_robot.position.y - pose_target_y)
		...
		if (d_x >= d_y): # == > lay truc Y lam puong
			if (abs(angle_toGoal) <= pi*0.25):
				...
				point_targetSecondary.y = pose_target_y
			...
			if (angle_toGoal >= 0):
				# print ("pi/2")
				point_targetSecondary.x = pose_target_x
		...
		angle_out = self.calculateAngle_twoPoint(pose_robot.position, point_targetSecondary.x, point_targetSecondary.y)
		
		dis_now = self.calculate_distance(pose_robot.position, point_targetSecondary)
		if (dis_now/2. > delta_distance):
			if (self.isNear_axis(pose_robot.position, pose_target_x, pose_target_y, 0.04) == 0 ):
	...
	def find_point_goal(self, X_s, Y_s, X_f, Y_f, a_qd, b_qd, c_qd, X_n, Y_n, dis_ahead):
		...
		if self.cur_goal_is == 2 or self.cur_goal_is == 3:
			X_g = X_f
			Y_g = Y_f
	...
	def control_navigation(self, X_point_goal, Y_point_goal, vel_x,theta):
		vel_th = 0.0
	...
	def run(self):
		...
		while not rospy.is_shutdown():
					...
					if self.end_of_list == True:
						...
						if self.is_target_change == True:
							...
							if f == -1:
								...
								# if self.list_x[f_next] > 500.0 or self.list_y[f_next] > 500.0:
								#     self.process =
				...
				if self.cur_goal_x < 500.0 and self.cur_goal_y < 500.0:
					...
					if self.is_need_turn_step1 == 1:
						...
						if self.check_circle == 1:
							...
							if self.zone_lidar.zone_circle_beside == 1 or self.zone_lidar.zone_circle_ahead_behind == 1:
							# if self.zone_lidar.zone_circle == 1:
								self.war_agv = 1
								rospy.logwarn('co vat can o 2 vung tron')
								self.stop()
							else:
								self.war_agv = 0
								gt = self.turn_ar(theta,self.tolerance_rot_step1,self.vel_rot_step1)
			...
			#----- dieu huong AGV
				...
				difference_x = abs(self.cur_goal_x - self.poseRbMa.position.x)
				difference_y = abs(self.cur_goal_y - self.poseRbMa.position.y)

				if (difference_x < difference_y):
					self.process = 8 # -- chon Y
					print ("chon Y")
				else:
					self.process = 9 # -- chon X
					print ("chon X")

			elif self.process == 8:
				twist = Twist()

				delta_distance = abs(self.poseRbMa.position.y -  self.cur_goal_y)
				...
				angle_goGoal = self.calculateAngle_twoPoint(self.poseRbMa.position, self.cur_goal_x, self.cur_goal_y)
				# --
				angle_robot = self.theta_rb_ht
				angle_goGoal_secondary = self.findAngle_targetSecondary_vs1(self.poseRbMa.position, point_2, angle_goGoal, delta_distance, 0.0)
				...
				if self.cur_goal_is == 2 and (self.distance_goal <= self.tol_simple or self.kc_con_lai <= self.tol_simple or self.is_over_goal == True) :
					...
					if self.is_check_zone == True:
						...
						if self.zone_lidar.zone_sick_ahead == 1:
							...
							v_th_send = self.control_navigation(self.x_td_goal, self.y_td_goal,vel_x,self.theta) 
				...
				if self.check_circle == 1:
					...
					# if self.zone_lidar.zone_circle == 1:
						self.war_agv = 1
						rospy.logwarn('co vat can o 2 vung tron truoc sau')
						self.stop()
					else:
						self.war_agv = 0
						gt = self.turn_ar(theta,self.tolerance_theta,self.vel_rot_step_f)
				...
				if s < fabs(self.kc_backward) :
					...
					if self.zone_lidar.zone_sick_behind == 1:
						self.war_agv = 1
						self.stop()
					else:
						self.war_agv = 0
