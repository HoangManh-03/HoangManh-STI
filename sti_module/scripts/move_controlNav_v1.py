...
from math import atan2, sin, cos, sqrt , fabs, acos, tan
...
class goalControl():
    def __init__(self):
        ...
        self.distance_goArc = rospy.get_param('~distance_goArc', 2.8)
    ...
    def stanley_controller(self, x_point, y_point, theta_point, vel_linear):
        # Tính toán Heading Error (HE)
        print(theta_point)
        theta_e = self.find_angleBW(theta_point, self.theta_rb_ht)

        # - Tinh toan sai so khoang cach
        dx = self.poseRbMa.position.x - x_point
        dy = self.poseRbMa.position.y - y_point
        ...
        # delta = theta_e + atan2(k_e * ss_dis * (-1) + theta_e, k_v * vel_linear)
        delta = k_eh * theta_e + atan2(k_ed * ss_dis * (-1), k_v * vel_linear)
        w = (vel_linear/L)*tan(delta)

        # w = k_ed * ss_dis * (-1) + k_v * vel_linear * sin(theta_e)

        print("theta_e: ", theta_e, " dis: ", ss_dis, "vel_linear: ", vel_linear, "w: ", w)
        return w
    ...
    def getEuler_fromStraightLine(self, a, b):
        angle_bt = 0.0
        if b == 0:
            if a < 0:
                angle_bt = PI/2.0
    ...
    def find_angleBW(self, angle_target, angle_reference):
        angle = angle_target - angle_reference
        if fabs(angle) >= PI:
            angle_fn = (2*PI - fabs(angle))
            if angle > 0:
                angle = -angle_fn
            else:
                angle = angle_fn
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
                ...
                if self.stt_agv == 0 : # AGV tinh goal tiep theo sau khi dung
                    ...
                    if self.end_of_list == True:
                        ...
                        if self.is_target_change == True:
                            ...
                            print("ID follow is " + str(idFollowValid))
            ...
            #----- dieu huong AGV
                ...
                self.theta_follow = self.getEuler_fromStraightLine(self.a_qd, self.b_qd)

                # print("theta_follow: ", self.theta_follow*180/PI)
                ...
                self.kc_con_lai = self.fnCalcDistPoints(self.X_n,\
                                                        self.cur_goal_x,\
                                                        self.Y_n,\
                                                        self.cur_goal_y)
                ...
                if self.is_need_pttt == 1 and self.distance_goal > self.dis_gt:
                    self.is_pre_pttt = 1
                    self.vel_x_now = self.ptgt(1.5,self.time_start_navi,0.0, v_x)
                ...
                if self.cur_goal_is == 2 and (self.distance_goal <= self.tol_simple or self.kc_con_lai <= self.tol_simple or self.is_over_goal == True) :
                    ...
                    if zonetim == -1 or zonenav == -1:
                        ...
                        if self.zone_lidar.zone_sick_ahead == 1 or self.zone_lidar.zone_sick_ahead == 2 or zonenav == 1:
                            ...
                            v_th_send = self.stanley_controller(self.X_n, self.Y_n, self.theta_follow, vel_x)
