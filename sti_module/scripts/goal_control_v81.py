...
class goalControl():
	def __init__(self):
		...
		self.tol_simple = 0.02      # do chinh xac theo truc x,y
		...
		self.angle_find_vel = 17.0*PI/180.0
	...
	def control_navigation(self, X_point_goal, Y_point_goal, vel_x, theta, dis):
		...
		# vel = vel_x/r
		# if self.cur_goal_is == 2 or self.cur_goal_is == 3:
		# 	vel = 1.4*fabs(self.theta_app)
		# 	if fabs(self.theta_app) < 0.005:
		# 		vel = 0
		# 	else:
		# 		if vel < 0.1:
		# 			vel = 0.1
	...
	def run(self):
		...
		while not rospy.is_shutdown():
			...
			#----- dieu huong AGV
				...
				self.theta_app = self.find_angle_between(self.a_qd, self.b_qd, self.theta_rb_ht)
