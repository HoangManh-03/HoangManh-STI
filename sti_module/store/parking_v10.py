...
class StiAutomaticParking(threading.Thread):
    def __init__(self, threadID):
        ...
        self.distance_max  = rospy.get_param('~distance_max',0.6)
        self.distance_min  = rospy.get_param('~distance_min',0.2)
        ...
        self.pub_status = rospy.Publisher('/parking_status', Parking_status, queue_size=10) 
    ...
    def run(self):
        global idTag_target,tag_g ,tt_x, tt_z, tt_g , find_id , id_legal , tolerance
        ...
        while not self.shutdown_flag.is_set():
          ...
          #B0 Wait thong tin tu server
            ...
            if buoc == -2 :
                rospy.loginfo("id: %s no legal ! --> find_id: %s",idTag_target,find_id)
                self.park_info(buoc,find_id,0,0,0) 
                if self.enable_parking == True:
                    self.enable_parking = False
                    rospy.sleep(0.1)
          ...
          #B9: tinh tien di vao      
            if buoc == 9 :
                ...
                if math.fabs(s) < (math.fabs(z_target)-distance_target) : 
                    if math.fabs(s) < 0.5:
                        twist.linear.x = 0.15
                        if tag_g > 0.015  : 
