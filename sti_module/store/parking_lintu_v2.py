...
from sti_msgs.msg import Parking_control , Parking_status , Zone_lidar_2head, Main_status
import math
import time
import threading
import signal
        ...
        while not self.shutdown_flag.is_set():
          ...
          # B111: check line tu 
            if buoc == 31 : 
                ...
                if self.Line_ahead > 2. and self.Line_ahead < 15. and math.fabs(x_target) < 0.5:
          ...
          # B777: check line tu 
            if buoc == 77 : 
                self.park_info(buoc,find_id,0,0,0) 
                print("self.Line_ahead = %s", self.Line_ahead )
                if self.Line_ahead > 2. and self.Line_ahead < 15. :
                    self.solanhoatdong = self.solanhoatdong + 1
