...
class QuadraticBezierCurves():
    ...
    def calculate_direction(self, t):
        # Tính đạo hàm của đường cong Bézier bậc 2 tại t
        dx = 2 * (1 - t) * (self.midPoint.x - self.pointOne.x) + 2 * t * (self.pointSecond.x - self.midPoint.x)
        dy = 2 * (1 - t) * (self.midPoint.y - self.pointOne.y) + 2 * t * (self.pointSecond.y - self.midPoint.y)
        return dx, dy
    
    def calculate_quaternion_from_direction(self, dx, dy):
        # Tính góc từ vector (dx, dy) trong mặt phẳng XY
        angle = atan2(dy, dx)
        
        # Chuyển góc thành quaternion (chỉ quay quanh trục Z)
        # quaternion_from_euler(x, y, z), với x, y, z là các góc quay quanh các trục X, Y, Z.
        # Trong trường hợp này, ta chỉ cần quay quanh trục Z.
    ...
    def slip(self):
        listPoint = np.empty((0,2), dtype=float)
        listQuaternion = np.empty((0, 4), dtype=float)  # Lưu trữ quaternion
        for i in self.t:
            ...
            dx, dy = self.calculate_direction(i)
            quaternion = self.calculate_quaternion_from_direction(dx, dy)

            listPoint = np.append(listPoint, np.array([[_x, _y]]), axis=0)

            listQuaternion = np.append(listQuaternion, np.array([quaternion]), axis=0)

        return listPoint, listQuaternion
...
class MakePathByPoint():
    def __init__(self):
        ...
        self.topic_subPose = rospy.get_param("~topic_subPose", "robotPose_nav")
        ...
        rospy.Subscriber(self.topic_subReq, LineRequestMove, self.callback_moveRequest, queue_size = 10)
        self.data_now_lineRequest = LineRequestMove() 
        ...
        self.pubListPoint = rospy.Publisher(self.topic_pubListP, ListPointRequestMove, queue_size = 1)
    ...
    def run(self):
        while not rospy.is_shutdown():    
            if self.is_moveRequest:
                ...
                data_now = copy.copy(self.data_lineRequest)
                # data_now = self.data_lineRequest

                # -- 
                self.msgListPoint.enable = data_now.enable
                self.msgListPoint.target_x = data_now.target_x
                self.msgListPoint.target_y = data_now.target_y
                self.msgListPoint.target_z = data_now.target_z
                self.msgListPoint.offset = data_now.offset
                self.msgListPoint.enableStop = data_now.enableStop
                self.msgListPoint.pointStopAvoid = data_now.pointStopAvoid
                self.msgListPoint.mission = data_now.mission
                self.msgListPoint.mess = data_now.mess

                # -- convert point to path
                if data_now.enable != 0:
                    # print("pose request", data.target_x, data.target_y, data.target_z)
                    poseTarget = PoseXYZ(data_now.target_x, data_now.target_y, data_now.target_z)
                    # print("pose request", data.target_x, data.target_y, data.target_z)
                    ...
                    numLen = len(data_now.pathInfo)
                    if numLen != 0:
                        listID = []
                        for i in range(numLen):
                            listID.append(data_now.pathInfo[i].pathID)

                        if listID != self.listIDPath:
                            self.listIDPath = listID
                            self.pathInfo = data_now.pathInfo
                ...
                if self.flagListPathChange or self.flagTargetChange:
                    ...
                    print("flagListPathChange: %s, flagTargetChange: %s" %(self.flagListPathChange, self.flagTargetChange))
                    self.resetAll()

                if data_now.enable == 1 and self.makePathDone == False:
                    num = len(data_now.pathInfo)
                    # xet stop point and point change direct
                    if num > 1:
                        for i in range(num-1):
                            directL1 = data_now.pathInfo[i].direction
                            directL2 = data_now.pathInfo[i+1].direction
                            typeP1 = data_now.pathInfo[i].typePath
                            typeP2 = data_now.pathInfo[i+1].typePath

                            if directL1 != directL2:
                                pointStop = StopPoint()
                                pointStop.pathID = data_now.pathInfo[i].pathID
                                pointStop.path2ID = data_now.pathInfo[i+1].pathID
                                pointStop.point = data_now.pathInfo[i].pointSecond
                                print("--------direct Point--------")
                            ...
                            if (typeP1 == self.straightLineMode and typeP2 == self.straightLineMode):
                                ...
                                point1.x = data_now.pathInfo[i].pointOne.position.x
                                point1.y = data_now.pathInfo[i].pointOne.position.y
                                point2.x = data_now.pathInfo[i].pointSecond.position.x
                                point2.y = data_now.pathInfo[i].pointSecond.position.y
                                point3.x = data_now.pathInfo[i+1].pointSecond.position.x
                                point3.y = data_now.pathInfo[i+1].pointSecond.position.y
                                ...
                                point2.x = data_now.pathInfo[i].pointSecond.position.x
                                point2.y = data_now.pathInfo[i].pointSecond.position.y
                                point3.x = data_now.pathInfo[i+1].pointMid.position.x
                                point3.y = data_now.pathInfo[i+1].pointMid.position.y

                            elif (typeP1 == self.quadraticBezierCurvesMode and typeP2 == self.straightLineMode):
                                isCheckAngle = True
                                angleValid = radians(self.angleStop)
                                point1.x = data_now.pathInfo[i].pointMid.position.x
                                point1.y = data_now.pathInfo[i].pointMid.position.y
                                point2.x = data_now.pathInfo[i].pointSecond.position.x
                                point2.y = data_now.pathInfo[i].pointSecond.position.y
                                point3.x = data_now.pathInfo[i+1].pointSecond.position.x
                                point3.y = data_now.pathInfo[i+1].pointSecond.position.y
                            ...
                            #         point1 = Point(data_now.pathInfo[i].pointOne.position.x, data_now.pathInfo[i].pointOne.position.y)
                            #         point2 = Point(data_now.pathInfo[i].pointSecond.position.x, data_now.pathInfo[i].pointSecond.position.y)
                            #         point3 = Point(data_now.pathInfo[i+1].pointSecond.position.x, data_now.pathInfo[i+1].pointSecond.position.y)
                            ...
                            #         point1 = Point(data_now.pathInfo[i].pointOne.position.x, data_now.pathInfo[i].pointOne.position.y)
                            #         point2 = Point(data_now.pathInfo[i].pointSecond.position.x, data_now.pathInfo[i].pointSecond.position.y)
                            #         point3 = Point(data_now.pathInfo[i+1].pointMid.position.x, data_now.pathInfo[i+1].pointMid.position.y)

                    # tao list point on path
                    for i in range(num):
                        pathID = data_now.pathInfo[i].pathID
                        if pathID:
                            typePath = data_now.pathInfo[i].typePath
                            if typePath == self.straightLineMode:
                                pointOne = Point(data_now.pathInfo[i].pointOne.position.x, data_now.pathInfo[i].pointOne.position.y)
                                pointSecond = Point(data_now.pathInfo[i].pointSecond.position.x, data_now.pathInfo[i].pointSecond.position.y)
                                ...
                                for j in listPoint:
                                    ...
                                    pointRQ.direction = data_now.pathInfo[i].direction
                                    pointRQ.movableZone = data_now.pathInfo[i].movableZone
                                    pointRQ.velocity = data_now.pathInfo[i].velocity
                                    pointRQ.fieldSafety = data_now.pathInfo[i].fieldSafety
                                    # position
                                ...
                                pointOne = Point(data_now.pathInfo[i].pointOne.position.x, data_now.pathInfo[i].pointOne.position.y)
                                pointSecond = Point(data_now.pathInfo[i].pointSecond.position.x, data_now.pathInfo[i].pointSecond.position.y)
                                pointCenter = Point(data_now.pathInfo[i].pointCenter.position.x, data_now.pathInfo[i].pointCenter.position.y)
                                radius = data_now.pathInfo[i].radius
                                ...
                                pointOne = Point(data_now.pathInfo[i].pointOne.position.x, data_now.pathInfo[i].pointOne.position.y)
                                pointSecond = Point(data_now.pathInfo[i].pointSecond.position.x, data_now.pathInfo[i].pointSecond.position.y)
                                pointMid = Point(data_now.pathInfo[i].pointMid.position.x, data_now.pathInfo[i].pointMid.position.y)
                                numberPts = data_now.pathInfo[i].numberPts
                                QBCLine =  QuadraticBezierCurves(pointOne, pointSecond, pointMid, numberPts)

                                listPoint, _ = QBCLine.slip()
                                print(_)

                                for index, j in enumerate(listPoint):
                                    point = PoseStamped()
                                    point.header.frame_id = 'map'
                                    point.header.stamp = rospy.Time.now()
                                    point.pose.position.x = j[0]
                                    point.pose.position.y = j[1]
                                    point.pose.position.z = 0.
                                    point.pose.orientation.z = _[index][2]
                                    point.pose.orientation.w = _[index][3]
                                    # print(point)
                                    self.msgPath.poses.append(point)
                                    ...
                                    # pointRQ.pose.orientation.x = 1.
                                    # pointRQ.pose.orientation.y = 1.
                                    pointRQ.pose.orientation.z = _[index][2]
                                    pointRQ.pose.orientation.w = _[index][3]
                ...
                print("pub lish", self.msgListPoint.enable)
                self.is_moveRequest = False
