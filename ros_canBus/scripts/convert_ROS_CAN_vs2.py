...
class CAN_ROS():
    def __init__(self):
        ...
        self.pub_statusHCU = rospy.Publisher("/POWER_info", POWER_info, queue_size = 200)
        self.POWER_info = POWER_info()
        # --
        self.pub_statusLift = rospy.Publisher("/lift_status", Lift_status, queue_size = 20)
        self.lift_status = Lift_status()
        # -- 
        self.pub_statusPSU = rospy.Publisher("/HC_info", HC_info, queue_size = 20)
    ...
    def syntheticFrame_sendCAN(self):
        ...
        if (self.CAN_sortSend == 0):
            ...
            self.CAN_dataSend.byte3 = self.POWER_control.sound_type
            self.CAN_dataSend.byte4 = 0b00111111
            self.CAN_dataSend.byte5 = self.data_controlFieldSick.data
        ...
        # -- MCU
            ...
            self.CAN_dataSend.byte3 = 1
            self.CAN_dataSend.byte4 = 1
            self.CAN_dataSend.byte5 = 1
    ...
    def analysisFrame_receivedCAN(self):
        ...
        if self.CAN_dataReceived.idSend == self.ID_HC:
            ...
            self.hc_info.status.data = self.CAN_dataReceived.byte0
            self.hc_info.zone_sick_ahead.data = self.syntheticFrame_Sick(int(self.CAN_dataReceived.byte1))
            self.hc_info.zone_sick_behind.data = self.syntheticFrame_Sick(int(self.CAN_dataReceived.byte2))

            self.pub_statusSick.publish(self.Sick_status)
            # print ("--- HC")
        ...
        # -- PSU
            ...
            self.PSU_status.voltages_analog = (self.CAN_dataReceived.byte2<<8)|(self.CAN_dataReceived.byte1)
        ...
        # -- OC
            ...
            self.lift_status.status.data 	    = int(self.CAN_dataReceived.byte0)
            self.lift_status.sensorLift.data  = self.CAN_dataReceived.byte1
            self.lift_status.sensorUp.data = self.CAN_dataReceived.byte2
            self.lift_status.sensorDown.data   = self.CAN_dataReceived.byte3
