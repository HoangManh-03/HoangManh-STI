...
# 			self.status_cancel = 1
	...
	def synthetic_error(self):
		...
		# -- load cell - ko ket noi
		if (self.detectLost_loadCell() == 1):
			listError_now.append(181)
		...
		# -- load cell - port
		if (self.status_port.loadcell == False):
			listError_now.append(183)

		# -- add 21/03/2022
		# -- mat ke khi dang di chuyen.
		# if self.check_lostLift_whenMove() == 1:
		# 	listError_now.append(143)
	...
	def run(self):
		if self.process == -1: # khi moi khoi dong len
			...
			if self.flag_cancelMission == 0:
				...
				self.statusU300L = self.statusU300L_cancelMission
