...
class ros_control():
	...
	# -- BY HAND:
			...
			# -- add 29/05/2024
			if self.app_button.bt_lift_up == True and self.app_button.bt_lift_down == False:
				self.flag_liftUp = 1
				self.flag_liftDown = 0
			
			elif self.app_button.bt_lift_up == False and self.app_button.bt_lift_down == True:
				self.flag_liftUp = 0
				self.flag_liftDown = 1

			if self.app_button.bt_lift_reset == True:
				self.flag_liftDown = 0
				self.flag_liftUp = 0
			...
			if self.step_tryTarget == 0 or self.step_tryTarget == 3:
				...
				# ------------------------------------------------------------
				# -- Lift - update 
				if self.flag_liftUp == 1 and self.flag_liftDown == 0:
					self.liftTask_byHand = self.liftUp
					self.flag_commandLift = 1

				elif self.flag_liftUp == 0 and self.flag_liftDown == 1:
					self.liftTask_byHand = self.liftDown
					self.flag_commandLift = 1

				elif self.flag_liftUp == 0 and self.flag_liftDown == 0:
					self.liftTask_byHand = self.liftStop
					self.flag_commandLift = 0
