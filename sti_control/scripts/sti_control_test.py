...
class ros_control():
	def __init__(self):
		...
		# -- wifi reset... + wifi respond		# Duc Anh update 20/9/2024
		rospy.Subscriber("/wifi_respond", Wifi_info, self.callback_wifiRespond)
		self.info_wifi = Wifi_info()
                
		self.pub_wifiRequest = rospy.Publisher("/wifi_request", Wifi_request, queue_size = 10)
		self.request_wifi = Wifi_request()
	...
	def callback_wifiRespond(self, data):
		self.info_wifi = data
...
#	Update error disconnect wifi	Le Duc Anh 20/09/2024
	def disconnectWifi(self):
		# delta_t = rospy.Time.now() - self.timeStampe_wifii

		if(self.info_wifi.status_wifi == False):
			return 1
		else:
			return 0
	...
	def synthetic_error(self):
		...
		#Update Error disconnect wifi - 20/09/2024
  
		#if self.disconnectWifi() == 1:
		...
		# -- MAIN: CAN ket noi 
		# if (self.main_info.CAN_status == 0):      # duong them 
		# 	listError_now.append(323)

		# -- lost Main 
		# if (self.detectLost_main() == 1):         # duong them
	...
	def run(self):
		if self.process == -1: # khi moi khoi dong len
			...
			if self.app_button.bt_clearError == 1 or self.main_info.stsButton_reset == 1:
				...
				# -- add 20/09/2022 : reset lai wifi

				if (self.info_wifi.status_wifi == False):
					self.request_wifi.reset = 1
				else:
					self.request_wifi.reset = 0
	...
	# -- BY HAND:
			...
			if self.step_tryTarget == 0 or self.step_tryTarget == 3:
				...
				# -- Send vel
					...
					if self.find_element(222, self.listError) == 1:     # lỗi 222 vẫn cho robot di chuyển
						if self.lift_status.status.data == 0 or self.lift_status.status.data >= 3:  # Đang thực hiện nhiệm vụ ở chế độ auto -> ko cho phép di chuyển.
							# -- Move
							if self.app_button.bt_remote == 0:
								self.enb_ps3.data = 0
			...
			if ( self.target_x != self.NN_cmdRequest.target_x ) or ( self.target_y != self.NN_cmdRequest.target_y) or ( self.target_z != self.NN_cmdRequest.target_z) or ( self.target_tag != self.NN_cmdRequest.tag):
				if (self.NN_cmdRequest.target_x < 500) and (self.NN_cmdRequest.target_y < 500):
					...
					if a1 == 1 or a2 == 1:
						...
						self.resetAll_variable()
						self.process = 43    # change here 42
				else:
	...
	# ------------------------------------------------------------------------------------
		...
		#reset wifi
		self.pub_wifiRequest.publish(self.request_wifi)
		self.rate.sleep()
