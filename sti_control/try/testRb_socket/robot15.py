...
class Communicate_socketIO():
	def __init__(self):
		rospy.init_node('sim_robot15', anonymous=False)
		...
		self.server_port = 4015

		self.AGV_IP = '192.168.1.100'
		self.AGV_port = 6004
		self.AGV_mac = "0c:9a:3c:07:bb:75"
		# -
		self.robot_x = -0.5
		self.robot_y = -0.5
		self.robot_r = radians(0)
		self.timeD = 1./8.
		# -
		self.vel_x = 0.15
		self.vel_r = 0.0
	...
	def run(self):
		if (self.step == 0):
			if (self.robot_x > 0.5):
				self.step = 1
				self.robot_r = radians(90)

		elif (self.step == 1):
			if (self.robot_y > 0.5):
				self.step = 2
				self.robot_r = radians(180)

		elif (self.step == 2):
			if (self.robot_x < -0.5):
				self.step = 3
				self.robot_r = radians(270)
...
#			#x = round(myObject.robot_x, 3)
			...
			battery = myObject.AGV_information.battery/10.
