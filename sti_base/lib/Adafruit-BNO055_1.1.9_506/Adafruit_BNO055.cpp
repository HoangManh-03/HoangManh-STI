...
#else BNO055_ACCEL_DATA_X_LSB_ADDR
  _wire->send(reg);
  _wire->send(value);
#endif
...
/* tuan add */
void Adafruit_BNO055::initialize() {
  Adafruit_BNO055(55);
  begin(0x55);
  // begin();
...
bool Adafruit_BNO055::testConnection(){

}

// ACCEL_*OUT_* registers
/* 1m/s^2 = 100 LSB */
void Adafruit_BNO055::getAcceleration(int16_t* x, int16_t* y, int16_t* z){
  uint8_t buffer[6];
  memset(buffer, 0, 6);
...
// GYRO_*OUT_* registers
 /*   1dps = 16 LSB  */
void Adafruit_BNO055::getRotation(int16_t* x, int16_t* y, int16_t* z){
  uint8_t buffer[6];
  memset(buffer, 0, 6);
...
// TEMP_OUT_* registers   
 /* 1uT = 16 LSB */    
void Adafruit_BNO055::getHeading(int16_t* mx, int16_t* my, int16_t* mz){
  uint8_t buffer[6];
  memset(buffer, 0, 6);
  ...
  *mx = ((int16_t)buffer[0]) | (((int16_t)buffer[1]) << 8);
  *my = ((int16_t)buffer[2]) | (((int16_t)buffer[3]) << 8);
  *mz = ((int16_t)buffer[4]) | (((int16_t)buffer[5]) << 8);
