...
namespace SickToolbox {
	...
	const std::string SickNav350::SetDataGet_COMMAND_TYPE="sMN";
	const std::string SickNav350::SetDataGet_COMMAND="mNPOSGetData";
	...
	const std::string SickNav350::GETDATANAVIGATION_COMMAND="mNPOSGetData"; // mNPOSGetPose
	...
	const std::string SickNav350::SHOWOPT_COMMAND_TYPE = "sWN";
	const std::string SickNav350::SHOWOPT_COMMAND = "NPOSPoseDataFormat"; // outputMode showOptParam - P26
	// -- add 
	const std::string SickNav350::SETREFTHRESHOLD_COMMAND_TYPE = "sWN";
	const std::string SickNav350::SETREFTHRESHOLD_COMMAND="NLMDReflThreshold";
	// -- add 
	const std::string SickNav350::LandmarkDataFormat_COMMAND_TYPE = "sWN";
	const std::string SickNav350::LandmarkDataFormat_COMMAND= "NLMDLandmarkDataFormat";
	...
	*/
	...
	void SickNav350::SetShowOption(int outputMode, int showOptParam)
	{
	  	std::cout << "set show opt refle" << std::endl;
	    uint8_t payload_buffer[SickNav350Message::MESSAGE_PAYLOAD_MAX_LENGTH] = {0};
	    int count = 0;
	    std::string command_type = this->SHOWOPT_COMMAND_TYPE;
	    std::string command = this->SHOWOPT_COMMAND;
	    ...
	    uint8_t byte_sequence[] = {115, 87, 65, 32, 78, 80, 79, 83, 80, 111, 115, 101, 68, 97, 116, 97, 70, 111, 114, 109, 97, 116};
	    ...
	    try {
			...
			std::cout<<"SetShowOption"<<std::endl;
	    }

	    catch(SickTimeoutException &sick_timeout_exception) {
			std::cerr << "SetShowOption: sick_timeout_exception" << std::endl;
			throw;
	    }

	    catch(SickIOException &sick_io_exception) {
			std::cerr << "SetShowOption: sick_io_exception" << std::endl;
			throw;
	    }

	    catch(...) {
			std::cerr << "SetShowOption: Unknown exception!" << std::endl;
			throw;
	    }
	...
	void SickNav350::SetLandmarkDataFormat(int Format, int ShowOptParam, int LandmarkFilter)
	{
	    uint8_t payload_buffer[SickNav350Message::MESSAGE_PAYLOAD_MAX_LENGTH] = {0};
	    int count = 0;
	    std::string command_type = this->LandmarkDataFormat_COMMAND_TYPE;
	    std::string command = this->LandmarkDataFormat_COMMAND;
	    ...
	    payload_buffer[count] = ' ';
	    count ++;
	    payload_buffer[count] = 48 + Format;
	    count ++;
	    ...
	    try {
			...
			std::cout<<"SetLandmarkDataFormat"<<std::endl;
	    }

	    catch(SickTimeoutException &sick_timeout_exception) {
			std::cerr << "SetLandmarkDataFormat: sick_timeout_exception" << std::endl;
			throw;
	    }

	    catch(SickIOException &sick_io_exception) {
			std::cerr << "SetLandmarkDataFormat: sick_io_exception" << std::endl;
			throw;
	    }

	    catch(...) {
			std::cerr << "SetLandmarkDataFormat: Unknown exception!" << std::endl;
			throw;
	    }
	...
	void SickNav350::SetDataGet(int wait, int Mask)
	{
	    uint8_t payload_buffer[SickNav350Message::MESSAGE_PAYLOAD_MAX_LENGTH] = {0};
	    int count=0;
	    std::string command_type=this->SetDataGet_COMMAND_TYPE;
	    std::string command=this->SetDataGet_COMMAND;
		...
		try {
			...
			// _ParseScanData();
			std::cout<< "SetDataGet" <<std::endl;
		}
	    catch(SickTimeoutException &sick_timeout_exception) {
			std::cerr << "SetDataGet: sick_timeout_exception" << std::endl;
			throw;
	    }

	    catch(SickIOException &sick_io_exception) {
			std::cerr << "SetDataGet: sick_io_exception" << std::endl;
			throw;
	    }

	    catch(...) {
			std::cerr << "SetDataGet - Unknown exception!" << std::endl;
			throw;
	    }
	}
	...
	void SickNav350::_ParseScanDataLandMark()
	{
		...
		if (arg[6] == "1"){
			std::cout << "reflector-count: " << atoi(arg[8].c_str()) << std::endl;
		}else{
			std::cout << "reflector-count: Not data"<< std::endl;
		}
		...
		// 	ReflectorData_.filter = atoi(arg[count++].c_str()); //<<std::endl;
			
		// 	int refcount = atoi(arg[count++].c_str());
		// 	ReflectorData_.num_reflector = refcount;
		// 	std::cout << "reflector-filter: " << ReflectorData_.filter << std::endl;
		// 	std::cout << "reflector-count: " << ReflectorData_.num_reflector << std::endl;
		...
		// 	}
		// }else{
		// 	std::cout << "No landmark data" << std::endl;
		// }
	...
	std::string SickNav350::_Convert_HexStr_To_BinStr(int num)// std::string num)
	{
		std::string out;
	...
	std::string SickNav350::_Convert_infoState_toBin(std::string num)
	{
		std::string out;
		for (int i = 0; i < num.length(); i ++){
			// std::cout << "BIN: " << char(num[i]) << std::endl;
			out += _Convert_HexStr_To_BinStr(num[i]); // _ConvertHexToDec
		}
		return out;
	}

	void SickNav350::GetSickMeasurements(double* range_values, unsigned int *num_measurements,
		double *sector_step_angle, double *sector_start_angle, double *sector_stop_angle,
		unsigned int *sector_start_timestamp, unsigned int *sector_stop_timestamp)
	{
		for (int i=0;i<MeasuredData_->num_data_points;i++)
		{
	...
	void SickNav350::_ParseScanDataNavigation()
	{
		...
		PoseData_.versionPositioning = _ConvertHexToDec(arg[2]);
		
		PoseData_.numberError = _ConvertHexToDec(arg[3]);
		if (arg[3]!="0")
		{
			std::cout << "Scan data unsuccessful"<<std::endl;
			return;
		...
		PoseData_.wait = _ConvertHexToDec(arg[4]);
		
		PoseData_.mask = _ConvertHexToDec(arg[5]);
		if (arg[5] < "1")
		{
			std::cout<<"Wrong selected signals"<<std::endl;
			return;
		...
		PoseData_.poseDataEnb = _ConvertHexToDec(arg[6]);
		...
		if (arg[count++] == "1")
		{
			...
			if (PoseData_.optionalPoseData == 1)
			{
				...
				// std::cout << "infoState: " << arg[count] << std::endl; // FFEE050F
		...
		if (arg[count++] == "1") // - landmark data follow
		{
		  	// std::cout<<"Landmark data follow"<<std::endl;
			...
			std::cout<<"No Landmark data follow"<<std::endl;
		}
	...
	void SickNav350::SetReflectorThreshold(uint8_t threshold)
	{
		std::cout<<"set reflector threshold command"<<std::endl;
	    uint8_t payload_buffer[SickNav350Message::MESSAGE_PAYLOAD_MAX_LENGTH] = {0};
	    int count=0;
	    std::string command_type=this->SETREFTHRESHOLD_COMMAND_TYPE;
	    std::string command=this->SETREFTHRESHOLD_COMMAND;
	    ...
	    char c[100];
	    sprintf(c,"%d",(int)threshold);
		for (int i = 0; i < strlen(c); i++)
		{
			payload_buffer[count]=c[i];
			count++;
			// std::cout<< "Set Reflector Threshold - Value: " << c[i] << std::endl;
		}

		std::cout<< "Set Reflector Threshold - Value: " << (int)threshold << std::endl;
	    // payload_buffer[count] = 48 + threshold;
	    // count++;
	    ...
	    try {
			...
			//sick_nav350_sector_data_t.
			// _SplitReceivedMessage(recv_message);
	    	std::cout<< "Set Reflector Threshold" <<std::endl;
	    }
	    catch(SickTimeoutException &sick_timeout_exception) {
			std::cerr << "Set Reflector Threshold - sick_timeout_exception" << std::endl;
			throw;
		}
		catch(SickIOException &sick_io_exception) {
			std::cerr << "Set Reflector Threshold - sick_io_exception" << std::endl;
			throw;
		}
		catch(...) {
			std::cerr << "Set Reflector Threshold - SickNav350::_set operating mode - Unknown exception!" << std::endl;
			throw;
		}
	}
