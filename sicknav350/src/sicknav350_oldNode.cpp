...
// #include <sicktoolbox/SickNAV350_old.hh>
#include "ros/ros.h"
...
int main(int argc, char *argv[]) {
	...
	try {
		...
		try {
			...
			// sick_nav350.SetOperatingMode(3); // landmark detection Mode
			// sick_nav350.SetOperatingMode(2); // mapping Mode
			// sick_nav350.SetOperatingMode(1); // standby (Default) Mode
			// sick_nav350.SetOperatingMode(0); // power down Mode
		/*
		0 = power down
		1 = standby (Default)
		2 = mapping
		...
		*/
		...
		while (ros::ok()) {
			...
			sick_nav350.GetDataNavigation(1, 1); //n(1, 1) 
