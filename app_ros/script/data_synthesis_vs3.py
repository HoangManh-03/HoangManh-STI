...
class data_synthesis():
	def __init__(self):
		...
		self.address_traffic = "192.168.1.34"
		# -- 
		self.listError = []
		self.listWarn = []
	...
	def convert_misson_vs2(self, val):
		switcher={
			0:'Không\nXác Định', # 
			1:'Nâng Kệ', # 
			2:'Hạ Kệ', # 
	...
	def convert_errorAll_vs2(self, val):
		switcher={
	...
	def show_job_vs2(self, val):
		job_now = ''
		switcher={
			0:'Không Xác Định', # 
			1:'Kiểm Tra Tự Động', # kiem tra trang thai ban nang sau khi Sang che do tu dong
			2:'Nhiệm Vụ Trước', # thuc hien nhiem vu truoc
			3:'Kiểm Tra Kệ', # kiem tra ke 
			4:'Đi Ra', # 
			5:'Di Chuyển', #
			6:'Đi Vào Kệ', # 
			7:'Nhiệm Vụ Sau', # 
	...
	def run(self):
		...
		while not rospy.is_shutdown():
			...
			# -- -- Mau sac khi sac pin
			if (self.main_info.charge_current > 0.5):
			...
			if (lenght_error == 0):
				...
				for i in range(lenght_error):
					if (self.infoAGV.listError[i] < 400):
						self.listError.append(self.convert_errorAll_vs2(self.infoAGV.listError[i]))

						str_errorDevice += self.convert_errorAll(self.infoAGV.listError[i])
						str_errorDevice += ' | '
					else:
						self.listWarn.append(self.convert_errorAll_vs2(self.infoAGV.listError[i]))
			...
			self.app_setValue.cb_listError = self.listError
			self.app_setValue.cb_listWarn = self.listWarn
			...
			# -- -- -- -- -- cmd
			self.app_setValue.lbv_job = self.show_job_vs2(self.infoAGV.process)
			# --
			self.app_setValue.lbv_job1 = self.convert_misson_vs2(self.NN_cmdRequest.before_mission)
			self.app_setValue.lbv_job2 = self.convert_misson_vs2(self.NN_cmdRequest.after_mission)
