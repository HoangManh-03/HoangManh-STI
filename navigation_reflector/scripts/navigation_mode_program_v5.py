...
*** Task Description: 
    ...
    + Sử dụng trọng số động cho chế độ init mode
        ...
        - Nêú ko tìm được thì báo lôĩ mất gương

    + Thêm dữ liệu vào r2000_data => publísh ros

    + Sửa sao cho vị trí robot ko bị mất: 

    + Tích hợp sử dụng map gương giống sicknav350 
...
class Navigation_mode():
    def __init__(self):
        ...
        # -- 
        self.distance_matching_error_threshold = 0.02  # before: 0.01 #  (zf = 2 cm ), do sai số vị trí các gương là +- 0.005 m => 0,014 m
        self.distance_threshold_increment = 0.002

        self.angle_matching_error_threshold = 0.05     # before: 0.02 # +- 0.005 m => 0.34 độ => 0.5 độ : 
        ...
        self.RMSE_THRESHOLD_NAVMODE = 1
        self.DISTANCE_COEF = 0.2
    ...
    def pub_lidar_pose_full(self, pub, ls_rms, ls_w, ls_nomatch, ls_samematch, ls_highrmse, x, y, phi, ls_localID, ls_globalID, process, no_ref):
        lidar_data = R2000_data()
        lidar_data.header.frame_id = "scanner_link"
        lidar_data.header.stamp = rospy.Time.now()
        ...
        while not rospy.is_shutdown():
            ...
            # -- Tìm ra vị trí khởi tạo cuả robot trên bản đồ và những gương nào đang được khớp với nhau
                ...
                for comb in combs:
                    ...
                    if degrees(theta_r) < 5 or degrees(theta_r) > 175:
                ...
                for item_theta_new in list_IDref_satify_fromtheta_new:
                    ...
                    id0_theta_ref = item_theta_new[1][0][0]
                    id1_theta_ref = item_theta_new[1][0][1]

                    id0_theta_det = item_theta_new[1][1][0]
                    id1_theta_det = item_theta_new[1][1][1]

                    id2_theta_ref = item_theta_new[2][0][1]
                    id2_theta_det = item_theta_new[2][1][1]

                    for item_d in list_IDref_satify_fromd:
                        id0_dis_ref = item_d[0][0]
                        id1_dis_ref = item_d[0][1]
                        id0_dis_det = item_d[1][0]
                        id1_dis_det = item_d[1][1]

                        # -- kc1
                        if id0_theta_ref == id0_dis_ref and id1_theta_ref == id1_dis_ref:
                            if id0_theta_det == id0_dis_det and id1_theta_det == id1_dis_det:
                                ls_IDmatch.append(id_theta)
                ...
                if len(ls_IDresult) < 3:
                    ...
                    print(f"Số gương khớp < 3 => yêu cầu tính toán lại lần thứ {self.count_cal_again_zeromatch} vơí hệ số khớp theo khoảng cách là {self.distance_matching_error_threshold}")
                    ...
                    for i in range(0, len(ls_IDresult)):
                        for j in range(i, len(ls_IDresult)):
                            if j != i:
                                ...
                                if idj_ref == idi_ref or idj_det == idi_det:
                    ...
                    if is_not_uniqueID:
                        ...
                        print(f"tồn tại gương trùng => yêu cầu tính toán lại lần thứ {self.count_cal_again_samematch} vơí hệ số khớp theo khoảng cách là {self.distance_matching_error_threshold}")

                        if self.distance_matching_error_threshold == 0:
                            self.process = -1
                            print("Ko thể tìm thấy gương => Error")
                        ...
                        if self.rmse_xy <= self.RMSE_THRESHOLD_INITMODE:        # sai số nhỏ hơn 2 cm theo x, y -> tạm thời cho nhỏ hơn 1 để debug
                            ...
                            print(f"rms > 0.02 => yêu cầu tính toán lại lần thứ {self.count_cal_again_highrmse} vơí hs khớp theo khoảng cách là {self.distance_matching_error_threshold}")                        

                # -- update data to ros
                self.ls_w[0] = self.distance_matching_error_threshold
                self.ls_w[1] = self.w_start
            ...
            # - Tìm ra vị trí robot sau khi đã tìm ra vị trí khởi tạo
                ...
                d_near = min(convert_posReflector_d)
                d_far = max(convert_posReflector_d)
                ...
                w_sigma = self.w_start + self.VEL_COEF*v_lidar       # sai số khoảng cách * sai số góc 
                ...
                if len(ls_detected_refFilter) < 3:
                    print("Số gương khớp nhỏ hơn 3 => navigation fail => init again")
                    ...
                    if self.rmse_xy <= self.RMSE_THRESHOLD_NAVMODE:
                        ...
                        print(f"-- Matching fail -> back to init mode --, tính toán lại lần thứ {self.count_highrmse} vơí trọng số là {self.distance_matching_error_threshold}")
                        self.process = 1
            ...
            # publish to ros
            self.pub_lidar_pose_full(self.lidar_pose_pub, self.ls_rmse, self.ls_w, self.ls_nomatch, self.ls_samematch, self.ls_highrmse, self.x_tf, self.y_tf, self.r_tf, self.ls_localID, self.ls_globalID, self.process, len(self.ls_localID))
