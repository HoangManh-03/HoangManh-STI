        ...
        while not rospy.is_shutdown():
            ...
            # -- Tìm ra vị trí khởi tạo cuả robot trên bản đồ và những gương nào đang được khớp với nhau
                ...
                for index, ref in enumerate(self.data_map.points):
                    ...
                    self.ls_reference_ref.append([(index + 1), d_ref, theta_ref])
                    self.ls_reference_ref_d.append(d_ref)
                    self.ls_reference_ref_theta.append(theta_ref)
                    ls_reference_ref.append([(index + 1), ref.x, ref.y])
                    self.ls_reference_ref_xy.append([(index + 1), ref.x, ref.y])
                    # ls_index.append(index + 1)
            ...
            # - Tìm ra vị trí robot sau khi đã tìm ra vị trí khởi tạo
                ...
                # print("\n")
                # for i in range(len(self.ls_reference_ref)):
                #     print(f"Gương tham chiếu thứ {self.ls_reference_ref[i][0]} có thông số là: d = {self.ls_reference_ref[i][1]}, theta = {self.ls_reference_ref[i][2]}")
                # print("--------------")                    
                ...
                # Đặt ngưỡng w_sigma
                w_sigma = 0.05*0.1  # sai số khoảng cách * sai số góc
                ...
                for ref in ls_detected_ref:
                    for i, id_ref in enumerate(valid_indices):
                        if id_ref != -1 and ref[0] == (i + 1):
                            ls_detected_refFilter.append([ref[1], ref[2]])
                print(ls_detected_refFilter)                
