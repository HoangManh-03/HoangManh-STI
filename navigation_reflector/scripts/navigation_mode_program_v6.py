...
*** Task Description: 
    ...
    - add 12/3/2025:
        + Chạy trên 11 gương tham chiếu => lỗi matching map (map cũ là 7 gương)
            - case 1: (2, 3, 4) và (1, 2, 5) => ko tìm được 4 và 5 do ko có khoảng cách thoả mãn
            - case 2: [(2, 3, 9), (3, 4, 5)] && [2, 3), (3, 4)], kết quả: (2, 3), (3, 4) -> sai, đúng là: (2, 4) và (3, 3)
        + 

    -  add 13/3/2025: 
        + Chế độ khởi tạo
            - Chỉ tính toán lại đoạn thay đổi giá trị ngưỡng => tăng tốc độ tính toán: 
            - Lâý full dữ liệu tại mảng => nếu có case lỗi thì test mâũ các trường hợp cái nào nhỏ thì lấy
            - Ưu tiên sử dụng gương từ trường hợp góc và 2 cạnh chung : OK
            - nếu chạy hết 1 vòng ngưỡng rồi
                + đẩy dữ liệu mới vào check

    - add 14/3/2025:
        + chế độ khởi tạo => thuật toán khớp gương mới: đang ok
        + Kiểm soát trạng thái báo lỗi của chương trình -> trước khi đẩy dữ liệu ra, cho các chương trình khác sử dụng
        + Thử gửi vị trí robot bù ra khi đang tìm vị trí của robot khi lùi vào kệ: OK ( tốc độ chậm thì oke)
        
        - Khi quay chỉ số rmse cũng nhiều hơn, khi tốc độ cao hơn
        - Nếu trùng dữ liệu thì có nên giảm ngưỡng w_sigma ko, hay tìm lại vị trí ???

        
*** Need to do
    + Tồn đọng trường hợp AGV khi init gương, nhưng các trường hợp đều xuất hiện 1 lần và tăng kiểu gì cũng không được
    (vẫn detect đủ 5 gương)
...
class Navigation_mode():
    def __init__(self):
        ...
        self.DISTANCE_THRESHOLD_INCREMENT = 0.005  # beforre 0.02
        ...
        self.ANGLE_THRESHOLD_ORIGIN = 0.01

        # for map nav350
        # self.ANGLE_THRESHOLD_MAX = 0.02
        # self.ANGLE_THRESHOLD_ORIGIN = 0.001

        self.ANGLE_THRESHOLD_INCREMENT = 0.015
        ...
        while not rospy.is_shutdown():
            ...
            # - Tìm ra vị trí robot sau khi đã tìm ra vị trí khởi tạo
                ...
                if (len(ls_localID) < 3):
                    ...
                    if abs(vx) < 0.005 and abs(vlidar_w) > 0.05: # robot quay
                        self.RMSE_THRESHOLD_NAVMODE = 2*abs(vlidar_w)
                    
                    else:
                        # -- setup nav mode
                        if self.data_NNinfoRespond.process == 6:   # agv di chuyển parking
                            self.RMSE_THRESHOLD_NAVMODE = 0.02
                        else:
                    ...
                    if rmse_xy <= self.RMSE_THRESHOLD_NAVMODE:
                        ...
                        self.ls_highrmse[0] = self.count_highrmse_navmode
