...
*** Task Description: 
    ...
    -  add 13/3/2025: 
        + Chế độ khởi tạo
            - so sánh rmse_xy sẽ theo angle_threshold : OK 
            ...
            - Viết cho 2 trường hợp lùi kệ và di chuyển thường
...
class Navigation_mode():
    def __init__(self):
        ...
        self.count_highrmse_parking = 0
        self.count_highrmse_moving = 0

        self.RMSE_THRESHOLD_INITMODE = 0.02              # before: 0.1
        ...
        while not rospy.is_shutdown():
            ...
            # -- Tìm ra vị trí khởi tạo cuả robot trên bản đồ và những gương nào đang được khớp với nhau
                ...
                if self.completed_initmode == 0:
                    ...
                    # -- clear same element in list_ID_notsure
                    ls_IDmatch_notsure = list(map(list, {tuple(map(tuple, sublist)) for sublist in ls_IDmatch_notsure}))

                    # Trích xuất từng cặp phần tử con
                    sub_pairs = [pair for sublist in ls_IDmatch_notsure for tpl in sublist for pair in tpl]

                    # Đếm số lần xuất hiện của từng cặp phần tử con
                    sub_pair_counts = Counter(sub_pairs)

                    # Tạo danh sách chứa các cặp cần giữ lại
                    filtered_pairs = {}

                    for pair1, count1 in sub_pair_counts.items():
                        keep = True  # Giả định giữ lại cặp này
                        for pair2, count2 in sub_pair_counts.items():
                            if pair1 != pair2 and (pair1[0] in pair2 or pair1[1] in pair2):
                                if count1 < count2:
                                    keep = False  # Nếu có phần tử chung nhưng ít hơn, thì loại bỏ
                                    break
                        if keep and count1 > 1:  # Chỉ giữ lại nếu số đếm > 1
                            filtered_pairs[pair1] = count1

                    print(filtered_pairs)

                    # Hiển thị kết quả
                    for pair, count in filtered_pairs.items():
                        ls_IDmatch.append(pair)
                        # print(f"{pair}: {count}")
                        # print(f"{pair}: {count}")

                    # # Tách các cặp số từ dữ liệu
                    # pairs = []
                    # for sublist in ls_IDmatch_notsure:
                    #     for tpl in sublist:
                    #         pairs.append((tpl[0], tpl[1]))
                    #         pairs.append((tpl[2], tpl[3]))

                    # # Đếm số lần xuất hiện của từng cặp số
                    ...
                    #     if count > 1:
                    #         ls_IDmatch_temp.append(pair)
                    #         ls_IDmatch.append(pair)
                    
                    # for idmatch in ls_IDmatch_temp:
                    #     for idmatch_notsure in ls_IDmatch_notsure:
                    #         if idmatch[0] == idmatch_notsure
                    ...
                    if len(ls_IDresult) < 3:
                        ...
                        if self.rmse_xy <= 2*self.angle_matching_error_threshold:
            ...
            # - Tìm ra vị trí robot sau khi đã tìm ra vị trí khởi tạo
                ...
                if self.data_NNinfoRespond.process == 6:   # agv di chuyển parking
                    ...
                    if len(ls_detected_refFilter) < 3:
                        print("Số gương khớp nhỏ hơn 3 => navigation mode in parking fail => init again")
                        ...
                        if self.rmse_xy <= self.RMSE_THRESHOLD_NAVMODE:
                            print("-- Matching good in navigation mode in parking ")
                            ...
                            self.ls_highrmse[0] = self.count_highrmse_parking

                            self.ls_rmse[0] = self.rmse_xy
                            
                        else:
                            self.count_highrmse_parking += 1
                            print(f"-- Navigation mode in parking fail -> back to init mode --, tính toán lại lần thứ {self.count_highrmse_parking} vơí trọng số là {self.distance_matching_error_threshold}")
                            self.completed_initmode = 0

                    # publish to ros
                    # if self.x_tf != 0 or self.y_tf != 0 or self.r_tf != 0:
                ...
                else:                 # agv di chuyển điểm thường 
                    self.RMSE_THRESHOLD_NAVMODE = 0.08
                    ...
                    if len(ls_detected_refFilter) < 3:
                        ...
                        if self.rmse_xy <= self.RMSE_THRESHOLD_NAVMODE:
                            ...
                            self.count_highrmse_moving += 1
                            print(f"-- Navigation mode fail -> back to init mode --, tính toán lại lần thứ {self.count_highrmse_moving} vơí trọng số là {self.distance_matching_error_threshold}")
                            self.completed_initmode = 0
                    ...
                    if self.x_tf != 0 or self.y_tf != 0 or self.r_tf != 0:
                        ...
                        self.ls_nomatch[1] = self.count_zeromatch
                        self.ls_samematch[1] = 0
                        self.ls_highrmse[1] = self.count_highrmse_moving
