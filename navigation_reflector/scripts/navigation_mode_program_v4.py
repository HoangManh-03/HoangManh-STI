...
*** Task Description: 
    ...
    + Áp dụng thuật toán bù chuyển động => theo tài liệu, đảm bảo robot hoạt động ở tốc độ cao, ko bị ảnh hưởng bởi quá trình tăng giảm tốc : OK 
    + Áp dụng công thức trọng số theo tốc độ di chuyển : OK
    + Sửa lại thuật toán match gương khi robot đứng yên, theo tài liệu
    + Thêm hệ thống báo navigation_status: OK
...
class Navigation_mode():
    def __init__(self):
        ...
        # -- 
        self.distance_matching_error_threshold = 0.02  # (zf = 2 cm ), do sai số vị trí các gương là +- 0.005 m => 0,014 m 
        self.angle_matching_error_threshold = 0.12     # +- 0.005 m => 0.34 độ => 0.5 độ
        ...
        self.count_cal_initmode_again = 0
    ...
    def run(self):
        while not rospy.is_shutdown():
            ...
            # -- Tìm ra vị trí khởi tạo cuả robot trên bản đồ và những gương nào đang được khớp với nhau
                ...
                valid_values = np.where(min_values < self.distance_matching_error_threshold, min_values, 0)
                ...
                # Tìm giá trị nhỏ nhất của từng cột
                min_values = np.min(G_angle, axis=0)

                # Tìm chỉ số hàng (vị trí) của giá trị nhỏ stivietnamnhất trong từng cột
                min_indices = np.argmin(G_angle, axis=0)
                ...
                # So sánh với ngưỡng distance 
                valid_values = np.where(min_values < self.angle_matching_error_threshold, min_values, 0)
                valid_indices = np.where(min_values < self.angle_matching_error_threshold, min_indices, -1)
                ...
                if is_not_uniqueID:
                    ...
                    del ls_IDresult[ls_index_id[0]]
                    del ls_IDresult[ls_index_id[1]-1]

                if len(ls_IDresult) < 3:
                    self.count_cal_initmode_again += 1
                    print(f"Số gương khớp < 3 => yêu cầu tính toán lại lần thứ {self.count_cal_initmode_again}")
                    ...
                    # self.ls_weight.append(self.w_start)

                    if self.rmse_xy <= 0.02:        # sai số nhỏ hơn 2 cm theo x, y -> tạm thời cho nhỏ hơn 1 để debug
                        ...
                        self.count_cal_initmode_again += 1
                        print(f"rms > 0.02 => yêu cầu tính toán lại lần thứ {self.count_cal_initmode_again}")                        
            ...
            # - Tìm ra vị trí robot sau khi đã tìm ra vị trí khởi tạo
                ...
                if len(ls_detected_refFilter) < 3:
                    ...
                    self.process = 1
                    # print(f"Tổng hợp trọng số sau {len(self.ls_weight)} la: {self.ls_weight}")
                    ...
                    if self.rmse_xy <= 1:
                        ...
                        print("-- Matching fail -> back to init mode --")
                        # time.sleep(5)
                        self.process = 1
