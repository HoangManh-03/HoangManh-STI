...
"""
    ...
    - Lặp lại 2 quy trình: Lộ trình 1 đường thẳng
        ...
        while not rospy.is_shutdown():
            if self.data_NNinfoRespond.mode == 2:                        # AGV ở chế độ tự động
                if self.process == 1:           # quy trinh 1
                    if self.subprocess == 1:  # -- nâng kệ ở 2 => hạ kệ ở 1
                        if time.time() - self.DieTime_pub < 3:                   
                            self.data_NNcmdRequest.process = 0.0
                            self.data_NNcmdRequest.target_id = 5
                            self.data_NNcmdRequest.target_x = -0.737
                            self.data_NNcmdRequest.target_y = -5.725
                            self.data_NNcmdRequest.target_z = -0.0229
                            self.data_NNcmdRequest.tag = 0
                            self.data_NNcmdRequest.offset = 1.2
                            self.data_NNcmdRequest.list_id = [3, 4, 5, 0, 0]
                            self.data_NNcmdRequest.list_x = [2.454, 1.463, -0.737, 0, 0]
                            self.data_NNcmdRequest.list_y = [0.544, 0.521, -5.725, 0, 0]
                    ...
                    elif self.subprocess == 2:    # -- nâng kệ ở 1
                        if time.time() - self.DieTime_pub < 3:                   
                            self.data_NNcmdRequest.process = 0.0
                            self.data_NNcmdRequest.target_id = 5
                            self.data_NNcmdRequest.target_x = -0.737
                            ...
                            self.data_NNcmdRequest.tag = 0
                            self.data_NNcmdRequest.offset = 1.2
                            self.data_NNcmdRequest.list_id = [5, 0, 0, 0, 0]
                            self.data_NNcmdRequest.list_x = [-0.737, 0, 0, 0, 0]
                            self.data_NNcmdRequest.list_y = [-5.725, 0, 0, 0, 0]
                            self.data_NNcmdRequest.list_speed = [100, 0, 0, 0, 0]
                    ...
                    elif self.subprocess == 3:                       # -- hạ kệ tại vị trí 2
                        if time.time() - self.DieTime_pub < 3:                   
                            self.data_NNcmdRequest.process = 0.0
                            self.data_NNcmdRequest.target_id = 3
                            self.data_NNcmdRequest.target_x = 2.454
                            self.data_NNcmdRequest.target_y = 0.544
                            self.data_NNcmdRequest.target_z = 3.133
                            self.data_NNcmdRequest.tag = 0
                            self.data_NNcmdRequest.offset = 1.2
                            self.data_NNcmdRequest.list_id = [5, 4, 3, 0, 0]
                            self.data_NNcmdRequest.list_x = [-0.737, 1.463, 2.454, 0, 0]
                            self.data_NNcmdRequest.list_y = [-5.725, 0.521, 0.544, 0, 0]
                            self.data_NNcmdRequest.list_speed = [100, 100, 100, 0, 0]
                            self.data_NNcmdRequest.before_mission = self.serverMission_liftUp
                            self.data_NNcmdRequest.after_mission = self.serverMission_liftDown
                            self.data_NNcmdRequest.id_command = 0
                        ...
                        if time.time() - self.DieTime_pub < 3:                   
                            ...
                            self.data_NNcmdRequest.tag = 0
                            self.data_NNcmdRequest.offset = 1.2
                            self.data_NNcmdRequest.list_id = [3, 0, 0, 0, 0]
                            self.data_NNcmdRequest.list_x = [2.454, 0, 0, 0, 0]
                            self.data_NNcmdRequest.list_y = [0.544, 0, 0, 0, 0]
                            self.data_NNcmdRequest.list_speed = [100, 0, 0, 0, 0]
                            self.data_NNcmdRequest.before_mission = self.serverMission_liftDown
                            self.data_NNcmdRequest.after_mission = self.serverMission_liftUp
                            self.data_NNcmdRequest.id_command = 0
