        ...
        while not rospy.is_shutdown():
            if self.data_NNinfoRespond.mode == 2:                        # AGV ở chế độ tự động
                if self.process == 1:           # quy trinh 1
                    if self.subprocess == 1:  # -- đi từ vị trí 1 -> id = 4
                        if time.time() - self.DieTime_pub < 3:                   
                            self.data_NNcmdRequest.process = 0.0
                            self.data_NNcmdRequest.target_id = 4
                            self.data_NNcmdRequest.target_x = -0.651
                            self.data_NNcmdRequest.target_y = 0.44
                            self.data_NNcmdRequest.target_z = -0.04608
                            self.data_NNcmdRequest.tag = 0
                            self.data_NNcmdRequest.offset = 1.2
                            self.data_NNcmdRequest.list_id = [1, 2, 3, 4, 0]
                            self.data_NNcmdRequest.list_x = [-0.278, -0.478, -0.578, -0.651, 0]
                            self.data_NNcmdRequest.list_y = [8.262, 6.262, 3.262, 0.44, 0]
                            ...
                            print("Gưỉ lệnh AGV chạy từ 1-> 4")
                    ...
                    elif self.subprocess == 2:    # -- đi từ vị trí 4 -> id 5
                        if time.time() - self.DieTime_pub < 3:                   
                            # self.data_NNcmdRequest.process = 0.0
                            # self.data_NNcmdRequest.target_id = 5
                            # self.data_NNcmdRequest.target_x = 1.46
                            # self.data_NNcmdRequest.target_y = 0.241
                            # self.data_NNcmdRequest.target_z = 1.52088
                            # self.data_NNcmdRequest.tag = 0
                            # self.data_NNcmdRequest.offset = 1.2
                            # self.data_NNcmdRequest.list_id = [4, 5, 0, 0, 0]
                            # self.data_NNcmdRequest.list_x = [-0.651, 1.46, 0, 0, 0]
                            # self.data_NNcmdRequest.list_y = [0.44, 0.241, 0, 0, 0]
                            ...
                            self.data_NNcmdRequest.target_z = 1.52088
                            self.data_NNcmdRequest.tag = 0
                            self.data_NNcmdRequest.offset = 0.0
                            self.data_NNcmdRequest.list_id = [4, 5, 0, 0, 0]
                            self.data_NNcmdRequest.list_x = [-0.651, 1.46, 0, 0, 0]
                            self.data_NNcmdRequest.list_y = [0.44, 0.241, 0, 0, 0]
                            self.data_NNcmdRequest.list_speed = [100, 100, 0, 0, 0]
                            ...
                            print("Gưỉ lệnh AGV chạy từ 4-> 5: ko offset")
                            self.pub_NNcmdRequest.publish(self.data_NNcmdRequest)
                            if self.data_NNinfoRespond.task_status == self.serverMission_liftUp and self.data_NNinfoRespond.process == 8:
                                self.subprocess = 3
                                self.DieTime_pub = time.time()
                                self.data_NNcmdRequest = NN_cmdRequest()

                    elif self.subprocess == 3:                       # -- đi từ id 4 -> id 5
                        if time.time() - self.DieTime_pub < 3:                   
                            # self.data_NNcmdRequest.process = 0.0
                            # self.data_NNcmdRequest.target_id = 7
                            # self.data_NNcmdRequest.target_x = 1.676
                            # self.data_NNcmdRequest.target_y = 5.298
                            # self.data_NNcmdRequest.target_z = 3.14
                            # self.data_NNcmdRequest.tag = 0
                            # self.data_NNcmdRequest.offset = 1.2
                            # self.data_NNcmdRequest.list_id = [5, 6, 7, 0, 0]
                            # self.data_NNcmdRequest.list_x = [1.46, 1.521, 1.676, 0, 0]
                            # self.data_NNcmdRequest.list_y = [0.241, 2.425, 5.298, 0, 0]
                            ...
                            print("Gưỉ lệnh AGV chạy từ 4-> 5 - co offset -> van nang")
                            self.pub_NNcmdRequest.publish(self.data_NNcmdRequest)
                            if self.data_NNinfoRespond.task_status == self.serverMission_liftUp and self.data_NNinfoRespond.process == 8:
                                self.subprocess = 4
                                self.DieTime_pub = time.time()
                    ...
                    elif self.subprocess == 4:                       # -- đi từ id 4 -> id 5
                        if time.time() - self.DieTime_pub < 3:                   
                            self.data_NNcmdRequest.process = 0.0
                            self.data_NNcmdRequest.target_id = 7
                            self.data_NNcmdRequest.target_x = 1.676
                            ...
                            print("Gưỉ lệnh AGV chạy từ 5-> 7")
                            self.pub_NNcmdRequest.publish(self.data_NNcmdRequest)
                            if self.data_NNinfoRespond.task_status == self.serverMission_liftDown and self.data_NNinfoRespond.process == 8:
                                self.subprocess = 5
                                self.DieTime_pub = time.time()
                                self.timeWait = time.time()
                                self.data_NNcmdRequest = NN_cmdRequest()
                    
                    elif self.subprocess == 5:                    # đi từ id 7 -> id 1
                        if time.time() - self.DieTime_pub < 3:                   
                            self.data_NNcmdRequest.process = 0.0
                            self.data_NNcmdRequest.target_id = 1
                            self.data_NNcmdRequest.target_x = -0.278
                            self.data_NNcmdRequest.target_y = 8.262
                            self.data_NNcmdRequest.target_z = -1.55648
                            self.data_NNcmdRequest.tag = 0
                            self.data_NNcmdRequest.offset = 1.2
                            self.data_NNcmdRequest.list_id = [7, 1, 0, 0, 0]
                            self.data_NNcmdRequest.list_x = [1.676, -0.278, 0, 0, 0]
                            self.data_NNcmdRequest.list_y = [5.298, 8.262, 0, 0, 0]
                            self.data_NNcmdRequest.list_speed = [100, 100, 0, 0, 0]
                            self.data_NNcmdRequest.before_mission = self.serverMission_liftDown
                            self.data_NNcmdRequest.after_mission = self.serverMission_liftUp
                            self.data_NNcmdRequest.id_command = 0
                            ...
                            print("Gưỉ lệnh AGV chạy từ 7 -> 1")
                            self.pub_NNcmdRequest.publish(self.data_NNcmdRequest)
