...
"""
    ...
    Version 4:        
        + Chế độ navigation:
            -> Thay vì matching theo từng giai đoạn vận tốc, chọn vận tốc khi tiến làm giá trị max. 
                + Nếu rmse cao hơn giá trị này => matching sai => xóa dữ liệu sai và chạy lại => nếu vẫn sai thì quay trở lại
                    chế độ init
                ...
                if self.step == 0:        # tính toán các thông tin nhận được từ gương tham chiếu và gương phát hiện
                    ...
                    if (len(ls_localID) < 3):
                        ...
                        if self.rmse_xy <= self.RMSE_THRESHOLD_NAVMODE:
                            ...
                            rospy.logwarn(f"-- Navigation mode is fail when spin -> back to init mode --, tính toán lại lần thứ {self.count_highrmse_navmode}") 

                            if self.count_highrmse_navmode >= 2:
                                self.count_highrmse_navmode = 0
                                self.completed_initmode = 0
                                self.completed_navmode = 0
