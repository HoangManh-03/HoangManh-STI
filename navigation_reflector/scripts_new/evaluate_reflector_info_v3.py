...
"""
    ...
    - Description: Get info of reflector from laser scan data like x, y, theta with parameter:
        ...
        + filter_width: 2
        + fitler_error_matching: strict
        + remission_filter_threshold: reflector_low
    
    - Note:
        + Các gương sẽ được sắp xếp theo góc quét tăng dần của lidar : -180 -> 180, CCW
        + Giá trị khoảng cách sẽ được cộng thêm 1 đoạn bằng bán kính của gương nữa => ra tâm gương được
        + Gương detect thế nào sẽ được pub ra như thế => bỏ việc pub data theo kiểu từ bé đến lớn => nó sẽ được xử lý trong code matching
"""
...
class Estimate_reflector_center():
    ...
    def callback_scan(self, data):
        if self.stop_flag == 0:
            ...
            self.ANGLE_TOLERANCE = data.angle_increment*6
            self.reflector_data = []
                ...
                if r < self.RANGE_THRESHOLD:
                    ...
                    if self.step == 0:
                        ...
                        if abs(delta_r) <= self.DISTANCE_TOLERANCE and abs(delta_a) <= self.ANGLE_TOLERANCE:
                            ...
                            # self.step = 0
                            mean_r = np.mean(self.ls_ranges) + self.radius_reflector
            ...
            #     print(f"reflector {i+1}: {ref.ranges:.5f} / {degrees(ref.theta):.5f}")
    ...
    def pub_marker(self, list_point):
        ...
        for i, p in enumerate(marker_a.points):
            ...
            text_marker.pose.position.z = p.z  # Hiển thị phía trên hình cầu
            text_marker.scale.z = 0.3  # Kích thước chữ  # 3.0
