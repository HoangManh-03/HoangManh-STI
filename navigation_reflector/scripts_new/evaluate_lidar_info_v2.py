...
"""
    ...
    Version 2:        
        + sử dụng thuật toán SVD + ICP thay cho SVD: => transform sai nhiều => bỏ qua
            - Nếu bạn biết matching chính xác và dữ liệu không quá nhiễu → SVD là đủ, không cần ICP.
            - Nếu dữ liệu phức tạp hơn, nhiễu hơn, hoặc matching không hoàn toàn chắc chắn → SVD làm init + ICP để refine (tinh chỉnh) là chiến lược tốt nhất.

        + use odom from wheel encoder and imu
    ...
    - Need to check: 
        ...
        + Create odom from imu + wheel encoders use robot localiztion
...
class Navigation_mode():
    ...
    def load_point_cloud_from_xy(self, points_xy):
        """
        Chuyển mảng numpy (N x 2) thành point cloud 3D (z = 0)
        """
        points_xyz = np.hstack((points_xy, np.zeros((points_xy.shape[0], 1))))
        pc = o3d.geometry.PointCloud()
        pc.points = o3d.utility.Vector3dVector(points_xyz)
        return pc

    def run_icp(self, source_pc, target_pc, threshold=0.5, init=np.eye(4)):
        """
        Chạy thuật toán ICP để khớp source với target.
        Trả về kết quả ICP hoặc None nếu lỗi.
        """
        try:
            if len(source_pc.points) == 0 or len(target_pc.points) == 0:
                raise ValueError("Source hoặc Target point cloud rỗng.")
            
            icp_result = o3d.pipelines.registration.registration_icp(
                source_pc, target_pc, threshold, init,
                o3d.pipelines.registration.TransformationEstimationPointToPoint()
            )
            return icp_result

        except Exception as e:
            print(f"[ICP ERROR] Lỗi khi chạy ICP: {e}")
            return None

    def ICP_algorithm(self, ls_pointMap, ls_pointRef): 
        svd_transform = self.SVD_algorithm(ls_pointMap, ls_pointRef)
        # print("Transformation from SVD:")
        # print(svd_transform)

        A = np.array(ls_pointMap) # map
        B = np.array(ls_pointRef) # gương

        # ======= Load point cloud và chạy ICP =======
        pc_ref = self.load_point_cloud_from_xy(A)
        pc_detect = self.load_point_cloud_from_xy(B)

        icp_result = self.run_icp(pc_detect, pc_ref, threshold=0.5, init=svd_transform)
