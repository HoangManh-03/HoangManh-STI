...
*** Task Description: 
    ...
    + Nếu muốn quét map đúng, bắt buộc hệ số rmse tại chương trình program navigations < 0.02 m
...
class DetectReflector():
    def __init__(self):
        ...
        # -- load data map
        self.dir_mapReflector = '/home/stivietnam/catkin_ws/src/navigation_reflector/data_new/data_v2/map_reflector_sti.json'
        ...
        self.rmse_threshold = 0.02
        self.RMSE_OFFSET = 0.01
        ...
        rospy.Subscriber('/lidar_pose', PoseStamped, self.callback_lidarpose, queue_size = 10)
        self.data_lidarpose = PoseStamped()
        self.is_lidarpose = False
    ...
    def callback_lidarpose(self, data):
        self.data_lidarpose = data
        self.is_lidarpose = True
            ...
            if self.process == 0:
                ...
                if self.have_map_old == False:
                    ...
                    if self.num_refStart != len(ls_refFilter):
                        print(f"số lượng gương các lần đọc không phù hợp, gốc là:{self.num_refStart} và hiện tại là: {len(ls_refFilter)}")
                        continue
                    ...
                    print("add reflector successfully, current number reflector is ", numref_after)
                    return
                
                # -- Trường hợp đã có gương trong map
                else:
                    if self.is_lidarpose == False or self.is_rawReflector == False or self.is_r2000 == False:
                        self.rate.sleep()
                        continue
                    ...
                    # self.rmse_threshold = self.data_r2000.rmse[1] + self.RMSE_OFFSET

                    #--
                    lidar_pose_x = self.data_lidarpose.pose.position.x
                    lidar_pose_y = self.data_lidarpose.pose.position.y

                    quata = ( self.data_lidarpose.pose.orientation.x, self.data_lidarpose.pose.orientation.y, self.data_lidarpose.pose.orientation.z, self.data_lidarpose.pose.orientation.w )
                    euler = euler_from_quaternion(quata)
                    lidar_pose_z = euler[2]
                    ...
                    # --- nếu hệ số rmse > 3cm => không lấy toạ độ
                    if self.data_r2000.rmse[1] > self.rmse_threshold or self.data_r2000.rmse[0] > self.rmse_threshold:
                        print("Hệ số rmse lớn hơn ngưỡng cho phép")
                        self.rate.sleep()
                        continue

                    numave = 30
                    if self.num_getPoseReflector < numave:
                        self.data_storePose_x.append(lidar_pose_x)
                        self.data_storePose_y.append(lidar_pose_y)
                        self.data_storePose_phi.append(lidar_pose_z)
                    ...
                    # -- 
                    print(f"Number current detected reflector: {self.num_reflector} và number of matched ref map: {self.data_r2000.number_reflectors}")
                    if self.num_reflector > self.data_r2000.number_reflectors:
                        ...
                        # -- check toạ độ gương mới đã có trong map ban đâù hay chưa ?
                        ls_refNotMatchmap_filter = []

                        for id_ref, ref_match in enumerate(ls_refNotMatchmap):
                            for ref_origin in self.lsReflector_inMap:
                                # a = fabs(ref_origin.x - ref_match[0])
                                # b = fabs(ref_origin.y - ref_match[1])

                                # c = sqrt(a*a + b*b)

                                if fabs(ref_origin.x - ref_match[0]) < 0.2 and fabs(ref_origin.y - ref_match[1]) < 0.20:
                                # if c < self.rmse_threshold:
                                    ls_refNotMatchmap_filter.append(id_ref)
                            
                        print(f"Có {len(ls_refNotMatchmap_filter)} toạ độ đã tồn tại trong map ban đâù")

                        if len(ls_refNotMatchmap_filter) != 0:
                            # - clear reflector point has index in above list
                            indices_to_remove_set = set(ls_refNotMatchmap_filter)

                            # Duyệt qua danh sách và giữ lại các phần tử không nằm trong indices_to_remove
                            filtered_list = [element for index, element in enumerate(ls_refNotMatchmap) if index not in indices_to_remove_set]
                            ls_refNotMatchmap = filtered_list

                        
                        print(f"Cuối cùng {len(ls_refNotMatchmap)} toạ độ cần thêm là: {ls_refNotMatchmap}")
                        ...
                        # print()
                        # Thêm gương vào map
                        self.add_reflectorMap(ls_refNotMatchmap)
