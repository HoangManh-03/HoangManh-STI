...
class MAP():
    def __init__(self):
        ...
        # self.dir_mapReflector = '/home/stivietnam/catkin_ws/src/navigation_reflector/data_new/map_reflector_05042025.json'  # map directory
        self.dir_mapReflector = rospy.get_param('~dir_map', '')

        
        self.lsReflector_inMap = []  # list reflectỏr and its ID
        self.lsReflector_inMap_arrange = []

        self.case_pub = rospy.get_param('~case', 0)
        self.lower_bound = rospy.get_param('~start_id', 0)
        self.upper_bound = rospy.get_param('~end_id', -1)
        self.segment_pub = rospy.get_param('~segment_pub', 0)

        self.lsReflector_inMap_unit = []
    ...
    def pub_marker(self, list_point):
        ...
        for i, p in enumerate(marker_a.points):
            ...
            text_marker.scale.z = 0.5  # Kích thước chữ     # 5.0
    ...
    def run(self):
        while not rospy.is_shutdown():
            ...
            if self.process == 0:
                ...
                if self.case_pub == 1:             # -- sắp xếp lại vị trí các gương có khoảng cách từ nhỏ tới lớn
                    ls_d = []
                ...
                elif self.case_pub == 2:         # -- sắp xếp vị trí gương theo góc tăng dần từ start angle
                    ls_theta = []
                    for ref in self.lsReflector_inMap:
                        d_f = sqrt(ref.x*ref.x + ref.y*ref.y)
                        theta_f = atan2(ref.y, ref.x)

                        ls_theta.append(theta_f)
                    
                    sorted_values, sorted_indices = self.sort_list_with_indices(ls_theta)
                ...
                else:                              # -- có thế nào thì pub ra thế đó
                    self.lsReflector_inMap_arrange = self.lsReflector_inMap

                if self.segment_pub == 1:
                    self.lsReflector_inMap_unit = []
                    k = 1
                    reflector = reflectorMap()
                    for i, ref in enumerate(self.lsReflector_inMap_arrange):
                        if self.lower_bound < ref.id <= self.upper_bound:
                            reflector.id = k
                            reflector.x = ref.x
                            reflector.y = ref.y
                            self.lsReflector_inMap_unit.append(reflector)

                            k = k + 1
                            reflector = reflectorMap()

                    self.lsReflector_inMap_arrange = self.lsReflector_inMap_unit
            ...
            # -- Kiểm tra xem đã nhận được list các điểm gương hiện taị chưa
                ...
                msg_raw_reflector.num_reflector = len(self.lsReflector_inMap_arrange)
                ...
                for ref in msg_raw_reflector.reflectors:
                    print(f"Thông tin các gương: {ref.GlobalID} / {ref.Cart_X:.2f} / {ref.Cart_Y:.2f} / {ref.Polar_Dist:.2f} / {degrees(ref.Polar_Phi):.2f}")

                print(f"Số lượng gương tham chiếu là: {len(self.lsReflector_inMap_arrange)}")    
