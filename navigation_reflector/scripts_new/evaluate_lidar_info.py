...
"""
    ...
    - NOTE:
        ...
        + Test thêm các thuật toán khớp map khác: ransac, tính toán map: svd

        + Các case khớp gương ko cover được:
            - Nếu các gương detect và gương tham chiếu nằm trên cùng 1 đường thẳng => bổ sung thêm gương

        + Thêm đoạn chương trình: 
            - Khi robot ko init được vị trí => tăng góc và giữ nguyên khoảng cách: OK
            - Thêm hệ số ngưỡng RMSE sẽ follow theo vận tốc của robot : OK
            - Hệ số distance coef để tăng thêm khoảng cách d_near/ d_far bé thôi, để bù cho sai số của odometry

    - Need to check: 
        + Thêm các gương ảo vào map gương, check xem thuật toán đọc thế nào
        + Check lại cái công thức bù chuyển động thật chi tiết
        + Chỉnh sửa filter_width = 2 => test lại all : done => oke hơn, do mình thiếu +0.03 bán kính => OK
        + Create odom from imu + wheel encoders use robot localiztion => OK 
        + test ransac trong trường hợp các kết quả khớp đều ra bằng 1 => ko sử dụng nha
        + Thuật toán ICP + SVD => ko OK

        + Lần đầu tiên: phân cụm gương lớn thành các 20 cụm gương nhỏ, với số lượng gương tăng lần lượt là 3 (theo khoảng cách tăng dần)
            - Vi dụ: 0-> 20, 3-> 23
            
        + Sau khi bị mất map do lỗi matching => init lại gương từ vị trí lỗi (setpose lại tại vị trí lỗi)
"""
...
class Navigation_mode():
    def __init__(self):
        ...
        self.WEIGHT_COEF = rospy.get_param('~weight_coef', 25)
        ...
        self.RMSE_THRESHOLD_INITMODE = rospy.get_param('~rmse_threshold_initmode', 10.0)
        ...
        #-- odom from combine wheel encoder + imu
        rospy.Subscriber('/odometry', Odometry, self.callback_odometry, queue_size = 10)
        self.data_odometry = Odometry()
        self.is_recv_odometry = False
    ...
    def callback_odometry(self, data):
        self.data_odometry = data
        self.is_recv_odometry = True
    ...
    def cal_info_idmatch(self, idmatch, ls_detected_ref):
    ...
    def run(self):
        while not rospy.is_shutdown():
            ...
            if self.process == 0:
                ...
                if self.is_recv_odometry == True:
                    c_k = c_k + 1
                else:
                    self.log_mess("warn","Wait odom data from odom combine", c_k)
            ...
            # -- Tìm ra vị trí khởi tạo cuả robot trên bản đồ và những gương nào đang được khớp với nhau
                ...
                if self.completed_initmode == 0:
                    ...
                    if self.step == 0:                    # tính toán dữ liệu k/c và góc từ dữ liệu đâù vào
                    ...
                    elif self.step == 1:                   # tìm giá trị khớp gương từ list trên
                        ...
                        if len(ls_IDmatch) == 0:
                            ...
                            if n == 0:
                                if len(ls_IDmatch_notsure) > 1: # -- when elements of sub_pair_counts equal 1
                                    ...
                                    for idmatch_temp in ls_IDmatch_temp:
                                        xr, yr, phir, rmse_xy, rmse_dr, w_start, globalID, localID = self.cal_info_idmatch(idmatch_temp, ls_detected_ref)
                                    ...
                                    print(f"Giá trị rmse nhỏ nhất là: {min_rmse_xy}")
                                    self.rmse_xy = min_rmse_xy 
                                    if min_rmse_xy <= 5*self.angle_matching_error_threshold:
                                        ...
                                        self.ls_globalID = info_IDmatch_globalID[ik]
                                        ...
                                        # --
                                        self.rmse_start = self.rmse_xy * self.WEIGHT_COEF
                                        ...
                                        print(f"rms > {3*self.angle_matching_error_threshold} => yêu cầu tính toán lại lần thứ {self.count_cal_again_highrmse} => sử dụng dữ liệu đầu vào khác")                                       
                                        # -- receive new data
                                        self.process = 1
                                        self.step = 0
                                    ...
                                    if self.angle_matching_error_threshold > self.ANGLE_THRESHOLD_MAX:
                                        # -- reset varibles
                                        # self.distance_matching_error_threshold = self.DISTANCE_THRESHOLD_MAX
                                        self.angle_matching_error_threshold = self.ANGLE_THRESHOLD_MAX
                                        ...
                                        if self.count_cal_initmode > 100:
                                            self.count_cal_initmode = 0
                                ...
                                print(f"Giá trị rmse nhỏ nhất là: {min_rmse_xy}")
                                self.rmse_xy = min_rmse_xy 
                                if min_rmse_xy <= 3*self.angle_matching_error_threshold:
                                    ik = info_IDmatch_rmsexy.index(min_rmse_xy)
                            ...
                            # - tính các thông số
                            xr, yr, phir, rmse_xy, rmse_dr, w_start, globalID, localID = self.cal_info_idmatch(ls_IDresult, ls_detected_ref)

                            self.rmse_xy = rmse_xy    
                            if rmse_xy <= 3*self.angle_matching_error_threshold:
                                self.x_tf = xr
                                self.y_tf = yr
                                self.r_tf = phir
            ...
            # - Tìm ra vị trí robot sau khi đã tìm ra vị trí khởi tạo
                ...
                # -- use vel from odom combined
                vx = self.data_odometry.twist.twist.linear.x
                vy = self.data_odometry.twist.twist.linear.y
                thetaz = self.data_odometry.twist.twist.angular.z
                ...
                w_sigma = self.w_start + self.VEL_COEF*v_lidar + self.OMEGA_COEF*abs(vlidar_w)     # sai số khoảng cách * sai số góc 

                print(f"Vận tốc hiện tại của lidar: {v_lidar} và vt quay là {vlidar_w}. Trọng số: {w_sigma}")
                ...
                if (len(ls_localID) < 3):
                    ...
                    # if abs(vx) < 0.005 and abs(thetaz) > 0.05: # robot quay
                    #     self.RMSE_THRESHOLD_NAVMODE = 2*abs(thetaz)
                    
                    # else:
                    #     # -- setup nav mode
                    #     # if self.data_NNinfoRespond.process == 6:   # agv di chuyển parking
                    #     #     self.RMSE_THRESHOLD_NAVMODE = 0.06
                    #     # else:
                    #     #     self.RMSE_THRESHOLD_NAVMODE = 0.20
                    #     self.RMSE_THRESHOLD_NAVMODE = 2*abs(v_lidar)

                    # -- 
                    
                    self.RMSE_THRESHOLD_NAVMODE = self.rmse_start + abs(v_lidar) + abs(thetaz)

                    print(f"Trọng số ngưỡng cho check rmse hiện tại là: {self.RMSE_THRESHOLD_NAVMODE}")
                    # -- 
                    if rmse_xy <= self.RMSE_THRESHOLD_NAVMODE:
                        ...
                        # -- 
                        self.rmse_start = rmse_xy * self.WEIGHT_COEF
                        print(f"Trọng số ngưỡng cho check rmse kế tiếp là: {self.rmse_start}")
                        
                    else:
                        self.count_highrmse_navmode += 1
                        print(f"-- Navigation mode is fail -> back to init mode --, tính toán lại lần thứ {self.count_highrmse_navmode} vơí trọng số là {self.w_start}")
                        self.completed_initmode = 0
            ...
            if self.process > 0:
                if self.finding_status == 0:                # đang tìm kiếm vị trí
                    pass
                    # if self.data_NNinfoRespond.process == 6:   # agv di chuyển parking
                    #     print("---- Đang tìm kiếm vị trí --- => pub luôn dữ liệu dự đoán")
                    ...
                    #     # predict position after scanning period ts
                    #     x_predict = self.pre_x + vlidar_x*self.SCANNING_PERIOD
                    #     y_predict = self.pre_y + vlidar_y*self.SCANNING_PERIOD
                    #     r_predict = self.pre_r + vlidar_w*self.SCANNING_PERIOD

                    #     self.translation = (x_predict, y_predict, 0.0)
                    #     self.quanternion = self.euler_to_quaternion(r_predict)
                    #     self.br.sendTransform(self.translation, self.quanternion, rospy.Time.now(), self.ref_frame, self.origin_frame)

                    #     # -- update again 
                    #     self.pre_x = x_predict
                    #     self.pre_y = y_predict
                    #     self.pre_r = r_predict
                    # else:                              # các case khác thì dừng
                    #     pass
                
                else:                                       # đã tìm kiêms được vị trí
