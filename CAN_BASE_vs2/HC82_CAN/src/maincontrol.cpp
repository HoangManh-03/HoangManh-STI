...
void MainControl::MainInit() {
    ...
    pinMode(SELECT_FIELD_PIN, OUTPUT);
    delay(10);
...
void MainControl::CanRecevieHandle() {
    if(Can_Main->CAN_ReceiveFrom(ID_MAIN)) {
        if(ID_HC == Can_Main->GetByteReceived(0)){
            ledControl = Can_Main->GetByteReceived(1);
            sickSelectFeild = Can_Main->GetByteReceived(3);

            // LOG_NUM(sickSelectFeild);
            
            // LOG_NUM(ledControl);
            // LOG_NUM(cam_ledControl);
            // LOG_MESS("ID_HC OK");
        }
        // LOG_MESS("CAN_Receive ID_MAIN");
    }
}

/* Loop điều khiển vùng Sick */
void MainControl::SickFeildControler()
{
    if (sickSelectFeild == 0){
        digitalWrite(SELECT_FIELD_PIN, 1);
    }else{
        digitalWrite(SELECT_FIELD_PIN, 0);
    }
...
{    
    ...
    // Can_Main->SetByteTransmit(sickSelectFeild, 6); // - to Test
...
void MainControl::read_SICK(){
    ...
    z1t = pcf8574.readButton(hcSickTopPin1In);
    z2t = pcf8574.readButton(hcSickTopPin2In);
